...
public class UISecurityTest {
   ...
   * private UISecurity security = new UISecurity(); List<UIRepositoryRole> rroles; List<UIRepositoryUser> rusers;
  ...
  public void init() {
     ...
     * UIRepositoryUser(userInfo)); } security.setUserList(rusers); security.setRoleList(rroles);
     */
  }
  ...
  public void testAddUser() throws Exception {
    /*
     * try { security.setSelectedDeck(ObjectRecipient.Type.USER); UIRepositoryUser userToAdd = new UIRepositoryUser(new
     * UserInfo()); userToAdd.setName("newuser"); userToAdd.setPassword("newpassword");
     * userToAdd.setDescription("new description"); Set<RoleInfo> rolesToAssign = new HashSet<RoleInfo>();
     * rolesToAssign.add(ctoRole); rolesToAssign.add(isRole); rolesToAssign.add(adminRole);
     * rolesToAssign.add(authenticatedRole); userToAdd.setRoles(rolesToAssign); security.addUser(userToAdd);
     * assertEquals(security.getSelectedUser(), userToAdd); assertEquals(security.getSelectedDeck(),
     * ObjectRecipient.Type.USER); assertEquals(security.getUserList().size(), 5); } catch (Exception e) {
     * Assert.fail(); }
     */
  }
  ...
  public void testUpdateUser() throws Exception {
    /*
     * try { UIRepositoryUser userToAdd = new UIRepositoryUser(new UserInfo()); userToAdd.setName("newuser");
     ...
     * security.updateUser(selectedUser); assertEquals(selectedUser.getPassword(), "newpassword123");
     * assertEquals(selectedUser.getDescription(), "new description 123"); assertEquals(security.getSelectedUser(),
     * selectedUser); assertEquals(security.getUserList().size(), 5); } catch (Exception e) { Assert.fail(); }
     */
  }
  ...
  public void testAddRole() throws Exception {
    /*
     * try { security.setSelectedDeck(ObjectRecipient.Type.ROLE); UIRepositoryRole roleToAdd = new UIRepositoryRole(new
     * RoleInfo()); roleToAdd.setName("newrole"); roleToAdd.setDescription("new description"); Set<UserInfo>
     * usersToAssign = new HashSet<UserInfo>(); usersToAssign.add(suzyUser); usersToAssign.add(tiffanyUser);
     * usersToAssign.add(joeUser); security.addRole(roleToAdd); assertEquals(security.getSelectedRole(), roleToAdd);
     * assertEquals(security.getSelectedDeck(), ObjectRecipient.Type.ROLE); assertEquals(security.getRoleList().size(),
     * 9); } catch (Exception e) { Assert.fail(); }
     */
  }
  ...
  public void testUpdateRole() throws Exception {
    /*
     * try { UIRepositoryRole roleToAdd = new UIRepositoryRole(new RoleInfo()); roleToAdd.setName("newrole");
     ...
     * assertEquals(selectedRole.getDescription(), "new description 123"); assertEquals(security.getSelectedRole(),
     * selectedRole); assertEquals(security.getRoleList().size(), 9); } catch (Exception e) { Assert.fail(); }
     */
  }
