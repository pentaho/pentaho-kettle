 ...
 * can create are not configurable. But it does cache the services.
 *
 * @author mlowery
 */
public class WebServiceManager implements ServiceManager {
   ...
   * eliminate the static singleton dependency on
  ...
  /**
   * Backward-compatible constructor — uses
   * {@link BrowserAuthSessionHolder#getInstance()} as the credential provider.
   * {@link CredentialProvider}.
   */

  public WebServiceManager( String baseUrl, String username ) {
    this( baseUrl, username, BrowserAuthSessionHolder.getInstance() );
  }

  private static String message( String key, String... tokens ) {
    return BaseMessages.getString( org.pentaho.di.repository.pur.WebServiceManager.class, key, tokens );
  }
