...
public class PurRepositoryConnector implements IRepositoryConnector {
  ...
  public PurRepositoryConnector( PurRepository purRepository, PurRepositoryMeta repositoryMeta, RootRef rootRef, CredentialProvider credentialProvider ) {
  ...
  public PurRepositoryConnector( PurRepository purRepository, PurRepositoryMeta repositoryMeta, RootRef rootRef ) {
    this( purRepository, repositoryMeta, rootRef, defaultCredentialProvider() );
  }
  ...
  private CredentialProvider resolveCredentialProvider( CredentialProvider credentialProvider ) {
    return credentialProvider != null ? credentialProvider : defaultCredentialProvider();
  }
   ...
   * holder ({@code BrowserAuthSessionHolder}).
   *
   * <p>
  ...
  public synchronized RepositoryConnectResult connect( final String username ) throws KettleException {
    CredentialProvider resolvedCredentialProvider = resolveCredentialProvider( credentialProvider );
    String baseUrl = repositoryMeta.getRepositoryLocation().getUrl();

    boolean hasOAuth = resolvedCredentialProvider.findAccessToken( baseUrl )
      .filter( t -> !t.isBlank() ).isPresent();
    boolean hasSession = resolvedCredentialProvider.findSessionCookie( baseUrl )
  ...
  public synchronized RepositoryConnectResult connect( final String username, final String password )
    ...
    final CredentialProvider resolvedCredentialProvider = useSessionAuth
      ? new SpoonCredentialProvider( baseUrl )
      : resolveCredentialProvider( credentialProvider );
    serviceManager = new WebServiceManager( baseUrl, username, resolvedCredentialProvider );
