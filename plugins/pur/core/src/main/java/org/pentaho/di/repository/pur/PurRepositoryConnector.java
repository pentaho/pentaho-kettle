...
public class PurRepositoryConnector implements IRepositoryConnector {
  ...
  private CredentialProvider credentialProvider = BrowserAuthSessionHolder.getInstance();
  ...
  public synchronized RepositoryConnectResult connect( final String username ) throws KettleException {
    credentialProvider = BrowserAuthSessionHolder.getInstance();
  ...
  private CredentialProvider selectCredentialProvider( boolean useSessionAuth, String baseUrl,
                                                       String decryptedPassword ) {
    if ( useSessionAuth ) {
      return new SpoonCredentialProvider( baseUrl );
    }
    if ( decryptedPassword == null || decryptedPassword.isEmpty() ) {
      return BrowserAuthSessionHolder.getInstance();
    }
    return ExplicitBasicAuthCredentialProvider.getInstance();
  }
  ...
  public synchronized RepositoryConnectResult connect( final String username, final String password )
    ...
    final String decryptedPassword = useSessionAuth || password == null
      ? ""
      : Encr.decryptPasswordOptionallyEncrypted( password );
    if ( useSessionAuth ) {
      resolveSessionId( baseUrl );
    }

    credentialProvider = selectCredentialProvider( useSessionAuth, baseUrl, decryptedPassword );
  ...
  @VisibleForTesting
  void setCredentialProviderForTesting( CredentialProvider credentialProvider ) {
  ...
  @VisibleForTesting
  AbsSecurityProvider createSecurityProvider( IUser user ) {
    return new AbsSecurityProvider( purRepository, repositoryMeta, user, serviceManager );
  }

  @VisibleForTesting
  PurRepositorySecurityManager createSecurityManager( IUser user ) {
    return new AbsSecurityManager( purRepository, repositoryMeta, user, serviceManager, credentialProvider );
  }

  @VisibleForTesting
  Callable<Boolean> createAuthorizationTask( final RepositoryConnectResult result ) {
    return () -> {
      ...
      result.setSecurityProvider( createSecurityProvider( result.getUser() ) );
      if ( log.isBasic() ) {
        log.logBasic(
      ...
      result.setSecurityManager( createSecurityManager( result.getUser() ) );
  ...
  private Future<Boolean> buildAuthorizationFuture( ExecutorService executor,
                                                    final RepositoryConnectResult result ) {
    return executor.submit( createAuthorizationTask( result ) );
  }
