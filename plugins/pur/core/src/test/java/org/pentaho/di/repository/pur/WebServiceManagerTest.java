...
public class WebServiceManagerTest {
  ...
  @Test
  public void testIsSessionAuthEnabled_ValidSession_ReturnsTrue() throws Exception {
    SpoonSessionManager mockMgr = mock( SpoonSessionManager.class );
    AuthenticationContext mockCtx = mock( AuthenticationContext.class );
    when( mockMgr.getAuthenticationContext( BASE_URL ) ).thenReturn( mockCtx );
    when( mockCtx.isAuthenticated() ).thenReturn( true );
    ...
    try ( MockedStatic<SpoonSessionManager> mocked = mockStatic( SpoonSessionManager.class ) ) {
      ...
      boolean result = invokePrivate( "isSessionAuthEnabled", new Class<?>[ 0 ] );
  ...
  @Test
  public void testIsSessionAuthEnabled_NoSession_ReturnsFalse() throws Exception {
    SpoonSessionManager mockMgr = mock( SpoonSessionManager.class );
    when( mockMgr.getAuthenticationContext( BASE_URL ) ).thenReturn( null );
  ...
  @Test
  public void testGetJSessionId_ValidSession_ReturnsJSessionId() throws Exception {
    SpoonSessionManager mockMgr = mock( SpoonSessionManager.class );
    AuthenticationContext mockCtx = mock( AuthenticationContext.class );
    when( mockMgr.getAuthenticationContext( BASE_URL ) ).thenReturn( mockCtx );
    when( mockCtx.isAuthenticated() ).thenReturn( true );
    ...
    try ( MockedStatic<SpoonSessionManager> mocked = mockStatic( SpoonSessionManager.class ) ) {
      ...
      String result = invokePrivate( "getJSessionId", new Class<?>[ 0 ] );

      assertEquals( JSESSION_ID, result );
    }
  }

  @Test
  public void testGetJSessionId_NoSession_ReturnsNull() throws Exception {
    SpoonSessionManager mockMgr = mock( SpoonSessionManager.class );
    when( mockMgr.getAuthenticationContext( BASE_URL ) ).thenReturn( null );
