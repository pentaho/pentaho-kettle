...
public abstract class RepositoryTestBase extends RepositoryTestLazySupport {
  ...
  public void testDirectories() throws Exception {
    ...
    assertEquals( startDir.getPath()
        + ( startDir.getPath().endsWith( "/" ) ? "" : RepositoryDirectory.DIRECTORY_SEPARATOR ) + DIR_CONNECTIONS,
        connDir.getPath() );
    repository.createRepositoryDirectory( startDir, DIR_SCHEMAS );
    repository.createRepositoryDirectory( startDir, DIR_SLAVES );
  ...
  public void testTransformations() throws Exception {
    ...
    assertTrue( transMeta.getSlaveStepCopyPartitionDistribution().getOriginalPartitionSchemas().get( 0 ).getName()
        .startsWith( EXP_PART_SCHEMA_NAME ) );
    ...
    assertEquals( getDate( new Date() ), getDate( service.getTransformationLock( transMeta.getObjectId() )
        .getLockDate() ) );
  ...
  public void testPartitionSchemas() throws Exception {
    ...
    assertEquals( Arrays.asList( new String[] { EXP_PART_SCHEMA_PARTID_1, EXP_PART_SCHEMA_PARTID_2 } ),
        fetchedPartSchema.getPartitionIDs() );
  ...
  public void testClusterSchemas() throws Exception {
    ...
    ClusterSchema fetchedClusterSchema =
        repository.loadClusterSchema( clusterSchema.getObjectId(), repository.getSlaveServers(), null );
    ...
    fetchedClusterSchema =
        repository.loadClusterSchema( clusterSchema.getObjectId(), repository.getSlaveServers(), null );
    assertEquals( EXP_CLUSTER_SCHEMA_BASE_PORT_V2, fetchedClusterSchema.getBasePort() );
    fetchedClusterSchema =
        repository.loadClusterSchema( clusterSchema.getObjectId(), repository.getSlaveServers(), VERSION_LABEL_V1 );
    assertEquals( EXP_CLUSTER_SCHEMA_BASE_PORT, fetchedClusterSchema.getBasePort() );
  ...
  protected NotePadMeta createNotePadMeta( String note ) throws Exception {
    return new NotePadMeta( EXP_NOTEPAD_NOTE.concat( note ), EXP_NOTEPAD_X, EXP_NOTEPAD_Y, EXP_NOTEPAD_WIDTH,
        EXP_NOTEPAD_HEIGHT );
  }
  ...
  public static class JobEntryAttributeTesterJobEntry extends JobEntryBase implements Cloneable, JobEntryInterface,
    ...
    @Override
    public void loadRep( final Repository rep, final IMetaStore metaStore, final ObjectId idJobentry,
        final List<DatabaseMeta> databases, final List<SlaveServer> slaveServers ) throws KettleException {
    ...
    @Override
    public void saveRep( final Repository rep, final IMetaStore metaStore, final ObjectId idJob )
    ...
    public void loadXML( final Node entrynode, final List<DatabaseMeta> databases,
        final List<SlaveServer> slaveServers, final Repository rep, final IMetaStore metaStore )
  ...
  public static class TransStepAttributeTesterTransStep extends BaseStepMeta implements StepMetaInterface,
    ...
    public void saveRep( Repository rep, final IMetaStore metaStore, ObjectId idTransformation, ObjectId idStep )
      throws KettleException {
