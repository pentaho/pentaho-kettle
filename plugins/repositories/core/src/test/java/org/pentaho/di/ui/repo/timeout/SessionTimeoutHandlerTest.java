...
public class SessionTimeoutHandlerTest {
  ...
  @Test
  public void handleReturnsRetryResultWhenRetrySucceeds() throws Throwable {
    ObjectId[] expected = new ObjectId[ 0 ];
    when( repository.getDatabaseIDs( anyBoolean() ) ).thenReturn( expected );
    Method method = Repository.class.getMethod( "getDatabaseIDs", boolean.class );
    ...
    verify( repository, times( 1 ) ).getDatabaseIDs( anyBoolean() );
    verify( sessionTimeoutHandler, times( 0 ) ).showLoginScreen( any() );
  }
  ...
  public void handleThrowsOriginalWhenLoginCanceled() throws Throwable {
    ...
    // Retry fails with timeout → login canceled → handleLoginCanceled() throws KettleRepositoryLostException
    sessionTimeoutHandler.handle( repository, original, method, new Object[] { Boolean.FALSE } );
  }
  ...
  @Test
  public void handleReturnsNullWithoutDoubleInvokeWhenMethodReturnsNull() throws Throwable {
    // When retryInvocationOnce succeeds with null, handle() should return null
    // immediately via short-circuit, without proceeding to the login flow.
    when( repository.getDatabaseIDs( anyBoolean() ) ).thenReturn( null );
    Method method = Repository.class.getMethod( "getDatabaseIDs", boolean.class );
    ...
    assertNull( result );
    // method.invoke is called exactly once by retryInvocationOnce — the retry succeeds
    // and handle() returns immediately without login or re-invocation.
    verify( repository, times( 1 ) ).getDatabaseIDs( anyBoolean() );
    verify( sessionTimeoutHandler, times( 0 ) ).showLoginScreen( any() );
  }
