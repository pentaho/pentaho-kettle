...
import static org.pentaho.di.core.plugins.PluginRegistry.log;

public class PentahoEnterpriseRepoFormComposite extends BaseRepoFormComposite {

  private static final String CLASS_NAME = "PentahoEnterpriseRepoFormComposite";
  ...
  protected Control uiAfterDisplayName() {
    ...
    txtUrl.addModifyListener( event -> {
      ...
      if ( Utils.isEmpty( txtUrl.getText() ) ) {
        ...
        setSsoStatusInfo( "Enter a repository URL to load SSO providers." );
        layout( true, true );
      ...
      comboSsoProvider.setEnabled( false );
      setSsoStatusInfo( "Loading SSO providers..." );
      layout( true, true );
      setSaveButtonEnabled();
      pendingProviderLoad = () -> {
    ...
    lblSsoProvider.setText( "Identity Provider" );
  ...
  public Map<String, Object> toMap() {
    ...
    if ( radioSSO.getSelection() ) {
      org.pentaho.di.core.logging.LogChannel log = new org.pentaho.di.core.logging.LogChannel( CLASS_NAME );
  ...
  private void loadSsoProviders( boolean preserveCurrentSelection ) {
    ...
    if ( Utils.isEmpty( serverUrl ) ) {
      clearProviderSelection( true );
      lblSsoStatus.setText( "Enter a repository URL to load SSO providers." );
      layout( true, true );
      setSaveButtonEnabled();
      return;
    }
    ...
    clearProviderSelection( true );
    comboSsoProvider.setEnabled( false );
    lblSsoStatus.setText( "Loading SSO providers..." );
    layout( true, true );
    setSaveButtonEnabled();

    final String urlToLoad = serverUrl;
    new Thread( () -> fetchAndApplyProviders( urlToLoad, preserveCurrentSelection, savedAuthorizationUri,
      savedRegistrationId ) ).start();
  }
  ...
  private void applyFetchedProviders( String urlToLoad, List<SsoProviderService.SsoProvider> finalProviders,
    ...
    if ( finalException != null ) {
      ...
      setSsoStatusError( "Unable to load SSO providers from the repository URL." );
    } else {
      ...
      if ( ssoProviders.isEmpty() ) {
        clearProviderSelection( true );
        setSsoStatusInfo( "No SSO providers are configured or enabled for this repository URL." );
      } else {
        restoreProviderSelection( preserveCurrentSelection );
        setSsoStatusInfo( "" );
      }
