...
@PluginDialog( id = "Constant", image = "CST.svg", pluginType = PluginDialog.PluginType.STEP,
  documentationUrl = "http://wiki.pentaho.com/display/EAI/Add+Constants" )
public class ConstantDialog extends BaseStepDialog implements StepDialogInterface {
  private static Class<?> PKG = ConstantDialog.class; // for i18n purposes, needed by Translator2!!
  ...
  private void ok() {
    ...
    for ( i = 0; i < nrfields; i++ ) {
      ...
      input.isEmptyString()[i] = BaseMessages.getString( PKG, "System.Combo.Yes" ).equalsIgnoreCase( item.getText( 10 ) );

      input.getFieldType()[i] = input.isEmptyString()[i] ? "String" : item.getText( 2 );
      ...
      input.getValue()[i] = input.isEmptyString()[i] ? "" : item.getText( 9 );
