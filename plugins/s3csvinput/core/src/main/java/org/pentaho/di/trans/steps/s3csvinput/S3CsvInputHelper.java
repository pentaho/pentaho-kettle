...
public class S3CsvInputHelper extends BaseStepHelper {
  ...
  public JSONObject listS3BucketsAction( TransMeta transMeta, Map<String, String> queryParams ) {
    ...
    try {
      ...
      response.put( ERROR, BaseMessages.getString( PKG, "S3CsvInputDialog.Exception.UnableToGetBuckets.Message", e.getMessage() ) );
      response.put( ACTION_STATUS, FAILURE_RESPONSE );
    }
    return response;
  }
  ...
  public JSONObject listS3ObjectsAction( TransMeta transMeta, Map<String, String> queryParams ) {
    ...
    try {
      ...
      if ( Utils.isEmpty( bucketName ) ) {
        response.put( ERROR, BaseMessages.getString( PKG, "S3CsvInputDialog.Exception.UnableToFindBucket.Message", bucketName ) );
        response.put( ACTION_STATUS, FAILURE_RESPONSE );
        return response;
      }
      S3ObjectsProvider provider = new S3ObjectsProvider( meta.getS3Client( transMeta ) );
      ...
      response.put( ERROR, BaseMessages.getString( PKG, "S3CsvInputDialog.Exception.UnableToGetFiles.Message", e.getMessage() ) );
      response.put( ACTION_STATUS, FAILURE_RESPONSE );
    }
    return response;
  }
  ...
  private void analyzeCsvAndBuildResponse( S3Object s3Object,
    ...
    try ( InputStreamReader reader = new InputStreamReader( s3Object.getObjectContent() );
      ...
      if ( fieldNames.length == 0 ) {
        response.put( ERROR, BaseMessages.getString( PKG, "S3CsvInputDialog.ErrorGettingFileDesc.DialogMessage" ) );
        response.put( ACTION_STATUS, FAILURE_RESPONSE );
        return;
      }
  ...
  public String[] extractFieldNamesFromCsv( InputStreamReader reader,
                                                   S3CsvInputMeta meta ) throws KettleFileException {
    String line = TextFileInput.getLine( log, reader, TextFileInputMeta.FILE_FORMAT_MIXED,
  ...
  private void stripEnclosuresFromFields( String[] fieldNames, String enclosure ) {
    for ( int i = 0; i < fieldNames.length; i++ ) {
