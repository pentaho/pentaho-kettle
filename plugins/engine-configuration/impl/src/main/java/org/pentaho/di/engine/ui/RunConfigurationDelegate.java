...
public class RunConfigurationDelegate {
  ...
  /**
   * Handle session expiry by prompting for reconnection via Spoon.
   *
   * @return true if successfully reconnected, false otherwise
   */
  private boolean handleSessionExpiry() {
    try {
      Spoon spoon = spoonSupplier.get();
      return spoon.handleSessionExpiryWithRelogin();
    } catch ( Exception e ) {
      showError( "Session expired", new Exception( "Your session has expired. Please reconnect to the repository." ) );
      return false;
    }
  }
