...
public class BlobBuffer {
  ...
  private static final transient Log log = LogFactory.getLog( BlobBuffer.class );
  ...
  protected void finalize() throws Throwable {
    try {
      ...
      log.debug( "Error on close in BlobBuffer.finalize()" );
   ...
   * <p/>
   * This method may fail due to security exceptions or local disk problems, in which case the blob storage will remain
   * entirely in memory.
   */
   ...
   * <p/>
   * A count of open and close requests is kept so that the file may be closed when no longer required thus keeping the
   ...
   * <p/>
   * The read pointer is partitioned into a page number and an offset within the page. This routine will read new pages
   * as required. The page size must be a power of 2 and is currently set to 1024 bytes.
   * 
   * @param readPtr
  ...
  public int read( int readPtr, byte[] bytes, int offset, int len ) throws IOException {
    ...
    } else if ( ( offset < 0 ) || ( offset > bytes.length ) || ( len < 0 ) || ( ( offset + len ) > bytes.length )
        || ( ( offset + len ) < 0 ) ) {
   ...
   * <p/>
   * The write pointer is partitioned into a page number and an offset within the page. This routine will write new
   * pages as required. The page size must be a power of 2 and is currently set to 1024 bytes.
   * 
   * @param writePtr
   *          the offset in the buffer of the required byte
   * @param b
    ...
    protected void finalize() throws Throwable {
      if ( open ) {
        try {
          ...
          log.debug( "Error on close in BlobBuffer.UnicodeInputStream.finalize()" );
          // Ignore closing anyway
        } finally {
          super.finalize();
        }
   ...
   * <p/>
   * This class interprets ASCII as anything which has a value below 0x80. This is more rigid than other drivers which
   * allow any character below 0x100 to be converted to returned. The more relaxed coding is useful when dealing with
   * most single byte character sets and if this behaviour is desired, comment out the line indicated in the read
   * method.
   */
  private class AsciiInputStream extends InputStream {
    ...
    protected void finalize() throws Throwable {
      if ( open ) {
        try {
          ...
          log.debug( "Error on close in Buffer.AsciiInputStream.finalize()" );
          // Ignore closing anyway
        } finally {
          super.finalize();
        }
    ...
    public int read() throws IOException {
      ...
      if ( b1 >= 0 ) {
        ...
        if ( b2 >= 0 ) {
          readPtr++;
          if ( b2 != 0 || b1 > 0x7F // Comment out this line for a more
    ...
    protected void finalize() throws Throwable {
      if ( open ) {
        try {
          ...
          // Ignore closing anyway
          log.debug( "Error on close in Buffer.BlobOutputStream.finalize()" );
        } finally {
          super.finalize();
        }
      }
  ...
  private class AsciiOutputStream extends OutputStream {
    ...
    /**
     * Costructs an ASCII <code>OutputStream</code> object over the BLOB buffer.
     * 
     * @param pos
     *          the starting position (from 0)
     * @throws IOException
    ...
    protected void finalize() throws Throwable {
      if ( open ) {
        try {
          ...
          log.debug( "Error on close in Buffer.AsciiOutputStream.finalize()" );
          // Ignore closing anyway
        } finally {
          super.finalize();
        }
   ...
   * 
   * @param pos
   *          the start position in the BLOB buffer (from 1)
   * @param len
  ...
  /**
   * Retrieve the BLOB data as an Big Endian Unicode <code>InputStream</code>.
   * 
   * @return the <code>InputStream</code> built over the BLOB data
   * @throws SQLException
   *           if an error occurs
  ...
  /**
   * Creates an <code>OutputStream</code> that can be used to update the BLOB.
   * <p/>
   * Given that we cannot know the final size of a BLOB created by the caller of this method, we assume the worst and
   * create a disk BLOB by default.
   * 
   * @param pos
   *          the start position in the buffer (from 1)
   * @param ascii
   ...
   * <li>The existing BLOB length is smaller or the same as the length of the new data
   ...
   *          the start position in the buffer (from 1)
   * @param bytes
   *          the byte array containing the data to copy
   * @param offset
   *          the start position in the byte array (from 0)
   * @param len
   *          the number of bytes to copy
   * @param copy
  ...
  public void truncate( long len ) throws SQLException {
    ...
    if ( len > this.length ) {
      throw new SQLException( BaseMessages.getString( PKG, "error.blobclob.lentoolong" ), "HY090" );
    }
   ...
   * 
   * @param pattern
   *          the byte array containg the search pattern
   * @param start
   *          the start position in the BLOB (from 1)
   * @return the <code>int</code> start index for the pattern (from 1) or -1 if the pattern is not found.
   * @throws SQLException
   *           if an error occurs
   */
