...
public class SQLParser {
  ...
  private int d;
  /**
   * Parameter list to be populated or <code>null</code> if no parameters are expected.
   */
  ...
  private static boolean[] identifierChar = { false, false, false, false, false, false, false, false, false, false,
    false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false,
    false, false, false, false, false, false, false, false, false, true, true, false, false, false, false, false,
    false, false, false, false, false, false, true, true, true, true, true, true, true, true, true, true, false, false,
  ...
  public static String[] parse( String sql, ArrayList<String> paramList, ConnectionJDBC3 connection,
      boolean extractTable ) throws SQLException {
  ...
  String[] parse( boolean extractTable ) throws SQLException {
    ...
    try {
      ...
      throw new SQLException( BaseMessages.getString( PKG, "error.parsesql.missing", String.valueOf( terminator ) ),
          "22025" );
    }
  ...
  private void copyParam( String name, int pos ) throws SQLException {
    // if (params == null) {
    // throw new SQLException(BaseMessages.getString(PKG, "error.parsesql.unexpectedparam", String.valueOf(s)),
    // "2A000");
    // }
    //
  ...
  private void skipMultiComments() throws SQLException {
    ...
    do {
      if ( s < len - 1 ) {
        ...
        throw new SQLException( BaseMessages.getString( PKG, "error.parsesql.missing", "*/" ), "22025" );
      }
    } while ( block > 0 );
    out[d++] = in[s++];
  }
  ...
  private void escape() throws SQLException {
    ...
    if ( in[s] == '?' ) {
      ...
      if ( "call".equals( esc ) ) {
        ...
        throw new SQLException( BaseMessages.getString( PKG, "error.parsesql.syntax", "call", String.valueOf( s ) ),
            "22019" );
      }
    } else {
      while ( Character.isLetter( in[s] ) ) {
      ...
      if ( "call".equals( esc ) ) {
        ...
        if ( !getDateTimeField( timeMask ) ) {
          throw new SQLException( BaseMessages.getString( PKG, "error.parsesql.syntax", "time", String.valueOf( s ) ),
              "22019" );
        }
      } else if ( "d".equals( esc ) ) {
        if ( !getDateTimeField( dateMask ) ) {
          throw new SQLException( BaseMessages.getString( PKG, "error.parsesql.syntax", "date", String.valueOf( s ) ),
              "22019" );
        }
      } else if ( "ts".equals( esc ) ) {
        if ( !getDateTimeField( timestampMask ) ) {
          throw new SQLException( BaseMessages
              .getString( PKG, "error.parsesql.syntax", "timestamp", String.valueOf( s ) ), "22019" );
        }
      } else {
        throw new SQLException( BaseMessages.getString( PKG, "error.parsesql.badesc", esc, String.valueOf( s ) ),
            "22019" );
      }
    }
  ...
  private void mustbe( char c, boolean copy ) throws SQLException {
    if ( in[s] != c ) {
      throw new SQLException( BaseMessages.getString( PKG, "error.parsesql.mustbe", String.valueOf( s ), String
          .valueOf( c ) ), "22019" );
    }
   ...
   * <p/>
   * Characters > 127 are assumed to be unicode letters in other languages than english which is reasonable in this
   * application.
   * 
   * @param ch
