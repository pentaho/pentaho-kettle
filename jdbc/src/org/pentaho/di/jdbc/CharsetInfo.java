...
public final class CharsetInfo {
    ...
    try {
      ...
      if ( stream != null ) {
        ...
        for ( Enumeration<?> e = tmp.propertyNames(); e.hasMoreElements(); ) {
          ...
          CharsetInfo prevInstance = instances.get( value.getCharset() );
  ...
  /**
   * Retrieves the <code>CharsetInfo</code> instance asociated with the specified server charset.
   * 
   * @param serverCharset
  ...
  /**
   * Retrieves the <code>CharsetInfo</code> instance asociated with the specified LCID.
   * 
   * @param lcid
   *          the server LCID
   * @return the associated <code>CharsetInfo</code>
  ...
  /**
   * Retrieves the <code>CharsetInfo</code> instance asociated with the specified sort order.
   * 
   * @param sortOrder
  ...
  /**
   * Retrieves the <code>CharsetInfo</code> instance asociated with the specified collation.
   * 
   * @param collation
   *          the server LCID
   * @return the associated <code>CharsetInfo</code>
  ...
  public static CharsetInfo getCharset( byte[] collation ) throws SQLException {
    ...
    if ( collation[4] != 0 ) {
      ...
      charset =
          getCharsetForLCID( ( collation[2] & 0x0F ) << 16 | ( collation[1] & 0xFF ) << 8 | ( collation[0] & 0xFF ) );
    }

    if ( charset == null ) {
      throw new SQLException( BaseMessages.getString( PKG, "error.charset.nocollation", Support.toHex( collation ) ),
          "2C000" );
    }
  ...
  /**
   * Constructs a <code>CharsetInfo</code> object from a character set descriptor of the form: charset preceded by a
   * numeric value indicating whether it's a multibyte character set (&gt;1) or not (1) and a vertical bar (|), eg
   * &quot;1|Cp1252&quot; or &quot;2|MS936&quot;.
   * 
   * @param descriptor
