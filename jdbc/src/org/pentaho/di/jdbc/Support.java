...
public class Support {
  ...
  static BigDecimal normalizeBigDecimal( BigDecimal value, int maxPrecision ) throws SQLException {
    ...
    while ( value.abs().unscaledValue().compareTo( max ) > 0 ) {
      ...
      if ( scale < 0 ) {
        ...
        throw new SQLException( BaseMessages
            .getString( PKG, "error.normalize.numtoobig", String.valueOf( maxPrecision ) ), "22000" );
      }
   ...
   *          an object reference to the caller of this method; must be a <code>Connection</code>,
   *          <code>Statement</code> or <code>ResultSet</code>
   * @param x
   *          the data object to convert
   * @param jdbcType
   *          the required type constant from <code>java.sql.Types</code>
  ...
  static Object convert( Object callerReference, Object x, int jdbcType, String charSet ) throws SQLException {
    try {
      switch ( jdbcType ) {
        ...
        default:
          throw new SQLException( BaseMessages
              .getString( PKG, "error.convert.badtypeconst", getJdbcTypeName( jdbcType ) ), "HY004" );
      }

      throw new SQLException( BaseMessages.getString( PKG, "error.convert.badtypes", x.getClass().getName(),
          getJdbcTypeName( jdbcType ) ), "22005" );
    } catch ( NumberFormatException nfe ) {
      throw new SQLException( BaseMessages.getString( PKG, "error.convert.badnumber", getJdbcTypeName( jdbcType ) ),
          "22000" );
    }
  }
   ...
   * 
   * @param buf
   *          The buffer in which the data will be embeded.
   * @param value
   *          The data object.
   * @param isUnicode
   *          Set to <code>true</code> if Unicode strings should be used, else <code>false</code>.
   * @param connection
   *          The {@link ConnectionJDBC3} object.
   */
   ...
   * <p>
   * This convenience method calls {@link #linkException(Exception, Throwable)} and casts the result for cleaner code
   * elsewhere.
   * 
   * @param sqle
   ...
   * <p>
   * If running under JVM 1.4+ the <code>Throwable.initCause(Throwable)</code> method will be invoked to chain the
   * exception, else the exception is logged via the {@link Logger} class. Modeled after the code written by Brian
   * Heineman.
   * 
   * @param exception
  ...
  /**
   * Converts a LOB to the equivalent Java type, i.e. <code>Clob</code> to <code>String</code> and <code>Blob</code> to
   * <code>byte[]</code>. If the value passed is not a LOB object, it is left unchanged and no exception is thrown; the
   * idea is to transparently convert only LOBs.
   * 
   * @param value
  ...
  /**
   * Converts a LOB type constant to the equivalent Java type constant, i.e. <code>Types.CLOB</code> to
   * <code>Types.LONGVARCHAR</code> and <code>Types.BLOB</code> to <code>Types.LONGVARBINARY</code>. If the type passed
   * is not that of a LOB, it is left unchanged and no exception is thrown; the idea is to transparently convert only
   * LOB types.
   * 
   * @param type
   *          a {@link Types} constant defining a JDBC type, possibly a LOB
   * @return if the type was that of a LOB, the equivalent Java object type, otherwise the original type
   */
   ...
   * 
   * @return <code>true</code> if <code>os.name</code> starts with "windows", else <code>false</code>.
   */
  ...
  /**
   * Returns the connection for a given <code>ResultSet</code>, <code>Statement</code> or <code>Connection</code>
   * object.
   * 
   * @param callerReference
   *          an object reference to the caller of this method; must be a <code>Connection</code>,
