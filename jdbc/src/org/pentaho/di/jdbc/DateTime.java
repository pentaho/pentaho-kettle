...
public class DateTime {
   ...
   * 
   * @param ts
   *          <code>Timestamp</code> object representing the datetime
   * @throws SQLException
   ...
   * 
   * @param t
   *          <code>Time</code> object representing the datetime
   */
  DateTime( Time t ) {
    timeValue = t;
    GregorianCalendar cal = (GregorianCalendar) calendar.get();
   ...
   * 
   * @param d
   *          <code>Date</code> object representing the datetime
   * @throws SQLException
   *           if the Date is out of range
   */
  ...
  public void packDate() throws SQLException {
    ...
    date =
        day - 32075 + 1461 * ( year + 4800 + ( month - 14 ) / 12 ) / 4 + 367 * ( month - 2 - ( month - 14 ) / 12 * 12 )
            / 12 - 3 * ( ( year + 4900 + ( month - 14 ) / 12 ) / 100 ) / 4 - 2415021;
  }
