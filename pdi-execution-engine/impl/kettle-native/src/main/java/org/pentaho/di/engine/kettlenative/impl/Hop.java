...
public class Hop implements IHop {
  ...
  public Hop( TransHopMeta hopMeta, StepMeta stepMeta, TransMeta transMeta, Map<StepMeta, IOperation> operations,
  ...
  @Override public void setTo( IOperation to ) {

  }

  public static List<IHop> convertTo( TransMeta transMeta, StepMeta stepMeta, Map<StepMeta, IOperation> operations,
                                      Map<TransHopMeta, IHop> hops ) {
    List<TransHopMeta> toHopMetas = getTransHopMetas( transMeta, stepMeta, hop -> stepMeta.equals( hop.getToStep() ) );
    return getIHops( transMeta, stepMeta, operations, hops, toHopMetas );

  }

  public static List<IHop> convertFrom( TransMeta transMeta, StepMeta stepMeta, Map<StepMeta, IOperation> operations,
                                      Map<TransHopMeta, IHop> hops ) {
    List<TransHopMeta> toHopMetas = getTransHopMetas( transMeta, stepMeta, hop -> stepMeta.equals( hop.getFromStep() ) );
    return getIHops( transMeta, stepMeta, operations, hops, toHopMetas );

  }

  private static List<IHop> getIHops( TransMeta transMeta, StepMeta stepMeta, Map<StepMeta, IOperation> operations,
                                      Map<TransHopMeta, IHop> hops, List<TransHopMeta> toHopMetas ) {
    return toHopMetas.stream()
      .map( hopMeta -> hops.computeIfAbsent(
        hopMeta, thisHopMeta -> new Hop( thisHopMeta, stepMeta, transMeta, operations, hops ) ) )
      .collect( Collectors.toList());
  }

  private static List<TransHopMeta> getTransHopMetas( TransMeta transMeta, StepMeta stepMeta,
