...
public final class ComplexType implements java.io.Serializable {
    ...
    private final List _elementNames = new ArrayList();
    ...
    private void processAll(Element all) {
        // annotation?, element*
        List elements = DomUtils.getChildElementsByName(all, "element");
...
//        }
    ...
    private void processSequence(Element sequence) {
        // annotation?, (element | group | choice | sequence | any)*
        List elements = DomUtils.getChildElementsByName(sequence, "element");
        for (Iterator itr = elements.iterator(); itr.hasNext();) {
            processElement((Element) itr.next());
        }
