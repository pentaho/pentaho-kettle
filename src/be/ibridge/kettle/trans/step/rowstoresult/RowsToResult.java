...
public class RowsToResult extends BaseStep implements StepInterface
{
	...
	public RowsToResult(StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta, Trans trans)
	{
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException
	{
		...
		{
		    getTransMeta().setResultRows( data.rows );
		...
		if ((linesRead>0) && (linesRead%Const.ROWS_UPDATE)==0) logBasic(Messages.getString("RowsToResult.Log.LineNumber")+linesRead); //$NON-NLS-1$
	...
	public void run()
	{
		...
		{
			logError(Messages.getString("RowsToResult.Log.UnexpectedError")+debug+"' : "+e.toString()); //$NON-NLS-1$ //$NON-NLS-2$
			setErrors(1);
			stopAll();
		}
		finally
