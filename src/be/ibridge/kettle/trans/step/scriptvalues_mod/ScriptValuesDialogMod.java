			...
			if (r!=null){
				...
				for (int i=0;i<r.size();i++){
						...
						switch(v.getType()){
							...
							case Value.VALUE_TYPE_BIGNUMBER: strItemToAdd=v.getName()+".getBigNumber()"; break; //$NON-NLS-1$
							case Value.VALUE_TYPE_BINARY: strItemToAdd=v.getName()+".getBytes()"; break; //$NON-NLS-1$
							case Value.VALUE_TYPE_SERIALIZABLE: strItemToAdd=v.getName()+".getSerializable()"; break; //$NON-NLS-1$
							default: strItemToAdd=v.getName(); break;
						}
						TreeItem itemInputFields = new TreeItem(item, SWT.NULL);
						itemInputFields.setText(strItemToAdd);
