...
public class LDIFInputFieldPosition implements Cloneable
{
     ...
     */
    public LDIFInputFieldPosition(String name, int type)
    {
        this.name      = name;
        this.type      = type;
        this.elementNr = 1;
     ...
     */
    public LDIFInputFieldPosition(String name, int type, int elementNr)
    {
        this.name = name;
        this.type = type;
     ...
     * @param encoded
     */
    public LDIFInputFieldPosition(String encoded) throws KettleValueException
    {
        int equalIndex = encoded.indexOf("=");
        if (equalIndex<0)
        {
            throw new KettleValueException(Messages.getString("LDIFInputFieldPosition.Exception.InvalidXMLFieldPosition", encoded));
        }
     ...
     * @param path An ArrayList of LDIFInputFieldPosition
    ...
    public static final String encodePath(ArrayList path)
    {
        ...
        {
            LDIFInputFieldPosition pos = (LDIFInputFieldPosition)path.get(p); 
            String elementName = pos.toString();
            if (p>0) encoded+=LDIFInputField.POSITION_MARKER;
            encoded+=elementName;
        }
