...
public class LDIFInputDialog extends BaseStepDialog implements
	...
	public String open() {
		...
		wFilenameList = new TableView(wFileComp, SWT.FULL_SELECTION
		...
		wInclFilenameField = new TextVar(wContentComp, SWT.SINGLE | SWT.LEFT
				| SWT.BORDER);
		...
		wInclRownumField = new TextVar(wContentComp, SWT.SINGLE | SWT.LEFT
				| SWT.BORDER);
		...
		wFields = new TableView(wFieldsComp, SWT.FULL_SELECTION | SWT.MULTI,
	...
	private void preview() {
		try {
			...
			TransMeta previewMeta = TransPreviewFactory
					.generatePreviewTransformation(oneMeta, wStepname.getText());
				...
				if (!progressDialog.isCancelled()) {
					...
					PreviewRowsDialog prd = new PreviewRowsDialog(shell,
							SWT.NONE, wStepname.getText(), progressDialog
									.getPreviewRows(wStepname.getText()),
							loggingText);
					prd.open();
				}
			}
			...
			new ErrorDialog(
					shell,
					Messages
							.getString("LDIFInputDialog.ErrorPreviewingData.DialogTitle"),
					Messages
							.getString("LDIFInputDialog.ErrorPreviewingData.DialogMessage"),
					e);
		}
	}
