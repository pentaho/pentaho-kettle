            ...
            case Value.VALUE_TYPE_NUMBER:
                ...
                for (int x = 0; x < Const.getNumberFormats().length; x++)
                {
                    if (numberFormat[i][x])
                    {
                        ...
                        {
                            if (log.isDetailed()) log.logDetailed(toString(), "This is unexpected: parsing [" + minstr[i] + "] with format [" + Const.getNumberFormats()[x]
                                    + "] did not work.");
                        }
                    }
                }
                ...
                if (!Const.isEmpty(minstr[i])) 
                {
	                for (int x = 0; x < Const.getDateFormats().length; x++)
	                {
	                    if (dateFormat[i][x])
	                    {
	                        ...
	                        {
	                            if (log.isDetailed()) log.logDetailed(toString(), "This is unexpected: parsing [" + minstr[i] + "] with format [" + Const.getDateFormats()[x]
	                                    + "] did not work.");
	                        }
	                    }
	                }
