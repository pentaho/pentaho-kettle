...
package be.ibridge.kettle.job.entry.copyfiles;
...
public class JobEntryCopyFiles extends JobEntryBase implements Cloneable, JobEntryInterface
{
	...
	public JobEntryCopyFiles(String n)
	{
		...
		setID(-1L);
		setType(JobEntryInterface.TYPE_COPY_FILES);
	}
	...
	public Result execute(Result previousResult, int nr, Repository rep, Job parentJob) throws KettleException 
	{
		...
		if (arg_from_previous && rows!=null) // Copy the input row to the (command line) arguments
		{
			for (int iteration=0;iteration<rows.size();iteration++) 
			{
				...
				vsourcefilefolder[iteration] = resultRow.getValue(0).getString();
				vdestinationfilefolder[iteration] = resultRow.getValue(1).getString();
				vwildcard[iteration] = resultRow.getValue(2).getString();
	...
	private boolean ProcessFileFolder(String sourcefilefoldername,String destinationfilefoldername,String wildcard,Job parentJob,Result result)
	{
		...
		String realSourceFilefoldername = StringUtil.environmentSubstitute(sourcefilefoldername);
		String realDestinationFilefoldername = StringUtil.environmentSubstitute(destinationfilefoldername);
