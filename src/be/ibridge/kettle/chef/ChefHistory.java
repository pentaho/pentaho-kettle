            ...
            if (jobMeta.getLogConnection()!=null)
            {
                if (!Const.isEmpty(jobMeta.getLogTable()))
                {
                    ...
                    try
                    {
                        ...
                        Row params = new Row();
                        params.addValue(new Value("transname", jobMeta.getName())); //$NON-NLS-1$
                        ResultSet resultSet = database.openQuery("SELECT * FROM "+jobMeta.getLogTable()+" WHERE JOBNAME = ? ORDER BY ID_JOB desc", params); //$NON-NLS-1$ //$NON-NLS-2$
