...
import org.pentaho.di.spoon.TreeSelection;
		...
		if (tree.equals(selectionTree))
		{
			...
			for (int s = 0; s < selection.length; s++)
			{
				...
				case 0:
					break;
				case 1: // ------complete-----
					if (path[0].equals(SpoonInterface.STRING_TRANSFORMATIONS)) // the
																				// top
																				// level
					// Transformations
					// entry
					...
					if (path[0].equals(SpoonInterface.STRING_JOBS)) // the top
																	// level
																	// Jobs
					// entry
					{
				...
				case 2: // ------complete-----
					if (path[0].equals(SpoonInterface.STRING_BUILDING_BLOCKS)) // the
																				// top
																				// level
					// Transformations
					// entry
					{
						if (path[1].equals(SpoonInterface.STRING_TRANS_BASE))
						{
							object = new TreeSelection(path[1], StepPlugin.class);
						}
					}
					if (path[0].equals(SpoonInterface.STRING_TRANSFORMATIONS)) // Transformations
					// title
					{
						object = new TreeSelection(path[1], spoon.delegates.trans.getTransformation(path[1]));
					}
					if (path[0].equals(SpoonInterface.STRING_JOBS)) // Jobs
																	// title
					{
				...
				case 3: // ------complete-----
																	...
																	// title
					{
						...
						if (path[2].equals(SpoonInterface.STRING_JOB_ENTRIES))
				...
				case 4: // ------complete-----
					...
					if (path[0].equals(SpoonInterface.STRING_JOBS)) // The name
																	// of a job
					{
						JobMeta jobMeta = spoon.delegates.jobs.getJob(path[1]);
						if (jobMeta != null && path[2].equals(SpoonInterface.STRING_CONNECTIONS))
							object = new TreeSelection(path[3], jobMeta.findDatabase(path[3]), jobMeta);
						if (jobMeta != null && path[2].equals(SpoonInterface.STRING_JOB_ENTRIES))
				...
				case 5:
					...
					{
						TransMeta transMeta = spoon.delegates.trans.getTransformation(path[1]);
						if (transMeta != null && path[2].equals(SpoonInterface.STRING_CLUSTERS))
						{
		...
		if (tree.equals(coreObjectsTree))
		{
			...
			for (int s = 0; s < selection.length; s++)
			{
				...
				case 1:
					break; // nothing
				case 2: // Job entries
					if (path[0].equals(SpoonInterface.STRING_JOB_BASE))
					{
					...
					break;
				case 3: // Steps
					if (path[0].equals(SpoonInterface.STRING_TRANS_BASE))
					{
