	...
	public Result execute(Result previousResult, int nr)
	{
		...
		if (connection!=null)
		{
			...
			try
			{
				...
				if(sqlfromfile)
				{
					...
					try{
						...
						if(!SQLfile.exists()) 
						{
							log.logError(BaseMessages.getString(PKG, "JobSQL.SQLFileNotExist",realfilename));
			...
			catch(KettleDatabaseException je)
			{
				...
				log.logError( BaseMessages.getString(PKG, "JobSQL.ErrorRunJobEntry",je.getMessage()));
			}
			finally
			{
				db.disconnect();
		...
		{
			result.setNrErrors(1);
			log.logError( BaseMessages.getString(PKG, "JobSQL.NoDatabaseConnection"));
		}
