    ...
    for (int i = 0; i < resultRows.size() && result.getNrErrors() == 0; i++)
    {
      ...
      try
      {
        ...
        if (!Const.isEmpty(headerName)) {
            logDebug(BaseMessages.getString(PKG, "JobHTTP.Log.HeadersProvided"));
            for (int j = 0; j < headerName.length; j++) {
                if (!Const.isEmpty(headerValue[j])) {
                    connection.setRequestProperty(environmentSubstitute(headerName[j]), headerValue[j]);
                    logDebug(BaseMessages.getString(PKG, "JobHTTP.Log.HeaderSet", environmentSubstitute(headerName[j]), environmentSubstitute(headerValue[j])));
                }
            }
        }
