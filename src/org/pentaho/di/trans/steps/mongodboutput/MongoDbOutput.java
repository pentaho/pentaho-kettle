...
public class MongoDbOutput extends BaseStep implements StepInterface {
      ...
      if (m_meta.getUpsert()) {
        ...
        if (updateQuery != null) {
          ...
          if (insertUpdate != null) {
            WriteResult result = m_data.getCollection().update(updateQuery, insertUpdate, true, 
                m_meta.getMulti());
  ...
  protected void doBatch() throws KettleException {
    WriteResult result = m_data.getCollection().insert(m_batch);
    CommandResult cmd = result.getCachedLastError();
