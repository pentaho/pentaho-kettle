    ...
    implements StepMetaInterface
{
        ...
        throws KettleXMLException
    {
        try {
            ...
            int nrFieldMapping = XMLHandler.countNodes(
                stepnode,
                "fields_mapping"); //$NON-NLS-1$
            int nrTabIsEnable = XMLHandler.countNodes(
                stepnode,
                "tab_is_enable_mapping"); //$NON-NLS-1$
            allocate(nrKeyMapping, nrFieldMapping, nrTabIsEnable);

            for (int i = 0; i < nrKeyMapping; i++) {
                Node vnode = XMLHandler.getSubNodeByNr(
                    stepnode,
                    "keys_mapping", i); //$NON-NLS-1$

                fieldTableForKeys[i] = XMLHandler.getTagValue(
                    vnode,
                    "key_field_name"); //$NON-NLS-1$
                fieldStreamForKeys[i] = XMLHandler.getTagValue(
                    vnode,
            ...
            for (int i = 0; i < nrFieldMapping; i++) {
                Node vnode = XMLHandler.getSubNodeByNr(
                    stepnode,
                    "fields_mapping", i); //$NON-NLS-1$

                fieldTableForFields[i] = XMLHandler.getTagValue(
                    vnode,
                    "field_field_name"); //$NON-NLS-1$
                fieldStreamForFields[i] = XMLHandler.getTagValue(
                    vnode,
    ...
    public String getXML()
    {
        ...
        retval.append("    ").append(XMLHandler.addTagValue("operation", operation)); //$NON-NLS-1$ //$NON-NLS-2$
        retval.append("    ").append(XMLHandler.addTagValue("custom_sql", custom_sql)); //$NON-NLS-1$ //$NON-NLS-2$
        retval.append("    ").append(XMLHandler.addTagValue("sql_statement", sql_statement)); //$NON-NLS-1$ //$NON-NLS-2$

        for (int i = 0; i < fieldTableForKeys.length; i++) {
            retval.append("      <keys_mapping>").append(Const.CR); //$NON-NLS-1$
            retval.append("        ").append(XMLHandler.addTagValue("key_field_name", fieldTableForKeys[i])); //$NON-NLS-1$ //$NON-NLS-2$
            retval.append("        ").append(XMLHandler.addTagValue("key_stream_name", fieldStreamForKeys[i])); //$NON-NLS-1$ //$NON-NLS-2$
            retval.append("      </keys_mapping>").append(Const.CR); //$NON-NLS-1$
        }

        for (int i = 0; i < fieldTableForFields.length; i++) {
            retval.append("      <fields_mapping>").append(Const.CR); //$NON-NLS-1$
            retval.append("        ").append(XMLHandler.addTagValue("field_field_name", fieldTableForFields[i])); //$NON-NLS-1$ //$NON-NLS-2$
            retval.append("        ").append(XMLHandler.addTagValue("field_stream_name", fieldStreamForFields[i])); //$NON-NLS-1$ //$NON-NLS-2$
            retval.append("        ").append(XMLHandler.addTagValue("insert_or_update_flag", insOrUptFlag[i])); //$NON-NLS-1$ //$NON-NLS-2$
            retval.append("      </fields_mapping>").append(Const.CR); //$NON-NLS-1$
        }

        for (int i = 0; i < tabIsEnable.length; i++) {
            retval.append("      <tab_is_enable_mapping>").append(Const.CR); //$NON-NLS-1$
            retval.append("        ").append(XMLHandler.addTagValue("tab_is_enable", tabIsEnable[i])); //$NON-NLS-1$ //$NON-NLS-2$
            retval.append("      </tab_is_enable_mapping>").append(Const.CR); //$NON-NLS-1$
        }
    ...
    {
        try {
            databaseMeta = rep.loadDatabaseMetaFromStepAttribute(
                id_step,
                "id_connection",
                databases);
            ...
            int nrKeyMapping = rep.countNrStepAttributes(
                id_step,
                "keys_mapping"); //$NON-NLS-1$
            int nrFieldMapping = rep.countNrStepAttributes(
                id_step,
                "fields_mapping"); //$NON-NLS-1$
            int nrTabIsEnable = rep.countNrStepAttributes(
                id_step,
                "tab_is_enable_mapping"); //$NON-NLS-1$
            ...
            for (int i = 0; i < nrKeyMapping; i++) {
                fieldTableForKeys[i] = rep.getStepAttributeString(
                    id_step,
                    i,
                    "key_field_name"); //$NON-NLS-1$
                fieldStreamForKeys[i] = rep.getStepAttributeString(
                    id_step,
                    i,
                    "key_stream_name"); //$NON-NLS-1$
                if (fieldStreamForKeys[i] == null)
            ...
            for (int i = 0; i < nrFieldMapping; i++) {
                fieldTableForFields[i] = rep.getStepAttributeString(
                    id_step,
                    i,
                    "field_field_name"); //$NON-NLS-1$
                fieldStreamForFields[i] = rep.getStepAttributeString(
                    id_step,
                    i,
                    "field_stream_name"); //$NON-NLS-1$
                if (fieldStreamForFields[i] == null)
                    fieldStreamForFields[i] = fieldTableForFields[i];
                insOrUptFlag[i] = rep.getStepAttributeBoolean(
                    id_step,
                    i,
            ...
            for (int i = 0; i < nrTabIsEnable; i++) {
                ...
                tabIsEnable[i] = rep.getStepAttributeBoolean(
                    id_step,
                    i,
        ...
        try {
            ...
            if (fieldStreamForKeys != null) {
                ...
                for (int i = 0; i < fieldStreamForKeys.length; i++) {
                    ...
                    System.out.println(fieldStreamForKeys[i] + ":"
                        + fieldStream.getTypeDesc() + ":"
                        + fieldStream.getLength());
                    myString.append("cast(null as "
                        + getSQLDataType(fieldStream) + ") as \""
                        + fieldStreamForKeys[i] + "\"," + Const.CR);

                    if ((fieldStreamForKeys.length - 1) != i) {

                        myMatchCondtion.append("src." + fieldStreamForKeys[i]
                            + " = tgt." + fieldTableForKeys[i] + " AND"
                            + Const.CR);

                    } else {

                        myMatchCondtion.append("src." + fieldStreamForKeys[i]
                            + " = tgt." + fieldTableForKeys[i] + Const.CR);
                    }
                }
            ...
            if (fieldStreamForFields != null) {
                ...
                for (int i = 0; i < fieldStreamForFields.length; i++) {
                    ...
                    System.out.println(fieldStreamForFields[i] + ":"
                        + fieldStream.getTypeDesc() + ":"
                        + fieldStream.getLength() + ":"
                        + getSQLDataType(fieldStream));
            ...
            cursor.append(
                "select * from table ( applib.remote_rows (" + Const.CR)
                .append("cursor (" + Const.CR)
                .append("select" + Const.CR)
                .append(myString.toString())
                .append("from" + Const.CR)
                .append("(values(0))" + Const.CR)
                .append(")," + Const.CR)
                .append(port + "," + Const.CR)
                .append("false" + Const.CR)
                .append(")" + Const.CR)
                .append(")" + Const.CR);
                ...
                operation))
            {
                ...
                mysql.append("MERGE INTO" + Const.CR).append(
                    schemaName + "." + tableName + Const.CR).append(
                    "as tgt" + Const.CR).append("USING (" + Const.CR).append(
                    cursor.toString()).append(") as src ON" + Const.CR).append(
                ...
                mysql.append(myUpdateStatement.toString() + Const.CR).append(
                    ...
                    "INSERT" + "(" + tgtColumns.toString() + ") values("
                        + srcColumns + ")");
                // for INSERT
