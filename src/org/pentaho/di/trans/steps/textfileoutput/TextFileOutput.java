...
public class TextFileOutput extends BaseStep implements StepInterface {
  ...
  public synchronized boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
    ...
    if ( r != null && first ) {
      ...
      if ( !meta.isFileAppended() && ( meta.isHeaderEnabled() || meta
        .isFooterEnabled() ) ) { // See if we have to write a header-line)
      ...
      for ( int i = 0; i < meta.getOutputFields().length; i++ ) {
        ...
        if ( data.fieldnrs[ i ] < 0 ) {
          throw new KettleStepException(
            "Field [" + meta.getOutputFields()[ i ].getName() + "] couldn't be found in the input stream!" );
        }
      }
    }

    if ( ( r == null && data.outputRowMeta != null && meta.isFooterEnabled() ) || ( r != null && getLinesOutput() > 0
      && meta.getSplitEvery() > 0 && ( ( getLinesOutput() + 1 ) % meta.getSplitEvery() ) == 0 ) ) {
      ...
      if ( Const.isEmpty( v.getStringEncoding() ) ) {
        ...
        try {
          ...
          throw new KettleValueException(
            "Unable to convert String to Binary with specified string encoding [" + v.getStringEncoding() + "]", e );
        }
      }
    } else {
      byte[] text;
    ...
    try {
      ...
      if ( str != null && str.length > 0 ) {
        ...
        if ( v.isString() ) {
          ...
          } else if ( !meta.isEnclosureFixDisabled() && containsSeparatorOrEnclosure( str, data.binarySeparator,
  ...
  private List<Integer> getEnclosurePositions( byte[] str ) {
    ...
    if ( data.binaryEnclosure != null && data.binaryEnclosure.length > 0 ) {
      for ( int i = 0; i < str.length - data.binaryEnclosure.length + 1;
            i++ ) { //+1 because otherwise we will not find it at the end
  ...
  public String buildFilename( String filename, boolean ziparchive ) {
    return meta
      .buildFilename( filename, meta.getExtension(), this, getCopy(), getPartitionID(), data.splitnr, ziparchive,
        meta );
  }
  ...
  public void openNewFile( String baseFilename ) throws KettleException {
    if ( baseFilename == null ) {
      throw new KettleFileException(
        BaseMessages.getString( PKG, "TextFileOutput.Exception.FileNameNotSet" ) ); //$NON-NLS-1$
    }
    ...
    try {
      if ( meta.isServletOutput() ) {
        ...
        if ( !Const.isEmpty( meta.getFileCompression() ) && !meta.getFileCompression()
          .equals( FILE_COMPRESSION_TYPE_NONE ) ) {
  ...
  private void createParentFolder( String filename ) throws Exception {
    ...
    try {
      ...
      if ( parentfolder.exists() ) {
        ...
        if ( isDetailed() ) {
          logDetailed(
            BaseMessages.getString( PKG, "TextFileOutput.Log.ParentFolderNotExist", parentfolder.getName() ) );
        }
        if ( meta.isCreateParentFolder() ) {
          parentfolder.createFolder();
          if ( isDetailed() ) {
            logDetailed(
              BaseMessages.getString( PKG, "TextFileOutput.Log.ParentFolderCreated", parentfolder.getName() ) );
          }
        } else {
          throw new KettleException( BaseMessages
            .getString( PKG, "TextFileOutput.Log.ParentFolderNotExistCreateIt", parentfolder.getName(), filename ) );
        }
      }
    } finally {
