...
public class CsvInput extends BaseStep implements StepInterface {
    ...
    try {
        ...
        if ( checkFeedback( getLinesInput() ) ) {
          if ( log.isBasic() ) {
            logBasic( BaseMessages
              .getString( PKG, "CsvInput.Log.LineNumber", Long.toString( getLinesInput() ) ) ); //$NON-NLS-1$
          }
        }
      }
      ...
      if ( getStepMeta().isDoingErrorHandling() ) {
        ...
        putError( data.outputRowMeta, e.getRowData(), e.getCauses().size(), errorDescriptions.toString(),
          errorFields.toString(), "CSVINPUT001" ); //$NON-NLS-1$
      } else {
    ...
    try {
      ...
      for ( int i = 0; i < data.fileSizes.size(); i++ ) {
        ...
        if ( data.startPosition >= totalFileSize && data.startPosition < totalFileSize + size ) {
          ...
          // remember where we started to read to allow us to know that we have to skip the header row in the next
          // files (if any)
          //
          data.startFilenr = i;
      ...
      if ( data.filenames.length > 0 ) {
        logBasic( BaseMessages
          .getString( PKG, "CsvInput.Log.ParallelFileNrAndPositionFeedback", data.filenames[ data.filenr ],
            Long.toString( data.fileSizes.get( data.filenr ) ), Long.toString( data.bytesToSkipInFirstFile ),
            Long.toString( data.blockToRead ) ) ); //$NON-NLS-1$
      }
    } catch ( Exception e ) {
      throw new KettleException( BaseMessages.getString( PKG, "CsvInput.Exception.ErrorPreparingParallelRun" ),
        e ); //$NON-NLS-1$
    }
  }
  ...
  private void getFilenamesFromPreviousSteps() throws KettleException {
      ...
      if ( firstRow ) {
        ...
        if ( index < 0 ) {
          throw new KettleException(
            BaseMessages.getString( PKG, "CsvInput.Exception.FilenameFieldNotFound", filenameField ) ); //$NON-NLS-1$
        }
      }
    ...
    logBasic( BaseMessages
      .getString( PKG, "CsvInput.Log.ReadingFromNrFiles", Integer.toString( data.filenames.length ) ) ); //$NON-NLS-1$
  }
  ...
  private boolean openNextFile() throws KettleException {
    try {
      ...
      if ( !( fileObject instanceof LocalFile ) ) {
        ...
        //
        throw new KettleException(
          BaseMessages.getString( PKG, "CsvInput.Log.OnlyLocalFilesAreSupported" ) ); //$NON-NLS-1$
      }
      ...
      if ( meta.isHeaderPresent() ) {
        if ( ( !data.parallel ) || // Standard flat file : skip header
          ...
          logBasic( BaseMessages
            .getString( PKG, "CsvInput.Log.HeaderRowSkipped", data.filenames[ data.filenr - 1 ] ) ); //$NON-NLS-1$
        }
      }
  ...
  /**
   * Check to see if the buffer size is large enough given the data.endBuffer pointer.<br> Resize the buffer if there is
   * not enough room.
   *
        ...
        while ( !delimiterFound ) {
          ...
          //
            ...
            ( !meta.isNewlinePossibleInFields() || outputIndex == meta.getInputFields().length - 1 ) &&
              ( data.crLfMatcher.isReturn( data.byteBuffer, data.endBuffer ) || data.crLfMatcher
  ...
  public static final String[] guessStringsFromLine( LogChannelInterface log, String line, String delimiter,
    ...
    try {
      ...
      while ( pos < length ) {
        ...
        if ( len_encl > 0 && line.substring( from, from + len_encl ).equalsIgnoreCase( enclosure ) ) {
          if ( log.isRowLevel() ) {
            log.logRowlevel( BaseMessages.getString( PKG, "CsvInput.Log.ConvertLineToRowTitle" ),
              BaseMessages.getString( PKG, "CsvInput.Log.ConvertLineToRow",
                line.substring( from, from + len_encl ) ) ); //$NON-NLS-1$ //$NON-NLS-2$
          }
          encl_found = true;
          int p = from + len_encl;
          ...
          if ( log.isRowLevel() ) {
            log.logRowlevel( BaseMessages.getString( PKG, "CsvInput.Log.ConvertLineToRowTitle" ),
              BaseMessages
                .getString( PKG, "CsvInput.Log.EndOfEnclosure", "" + p ) ); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
          }
        } else {
          encl_found = false;
          boolean found = false;
        ...
        if ( encl_found ) {
          ...
          if ( log.isRowLevel() ) {
            log.logRowlevel( BaseMessages.getString( PKG, "CsvInput.Log.ConvertLineToRowTitle" ),
              BaseMessages
                .getString( PKG, "CsvInput.Log.NormalFieldFound",
                  "" + pol ) ); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
          }
        }
      ...
      if ( pos == length ) {
        if ( log.isRowLevel() ) {
          log.logRowlevel( BaseMessages.getString( PKG, "CsvInput.Log.ConvertLineToRowTitle" ), BaseMessages.getString(
            PKG, "CsvInput.Log.EndOfEmptyLineFound" ) ); //$NON-NLS-1$ //$NON-NLS-2$
        }
