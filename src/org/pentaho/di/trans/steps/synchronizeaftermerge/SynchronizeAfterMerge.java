...
public class SynchronizeAfterMerge extends BaseStep implements StepInterface {
  ...
  private synchronized void lookupValues( Object[] row ) throws KettleException {
    ...
    try {
      if ( operation == null )
        throw new KettleException( BaseMessages.getString( PKG, "SynchronizeAfterMerge.Log.OperationFieldEmpty",
      ...
      if ( operation.equals( data.insertValue ) ) {
        ...
        if ( log.isRowLevel() )
          logRowlevel( BaseMessages.getString( PKG, "SynchronizeAfterMerge.InsertRow", row.toString() ) ); //$NON-NLS-1$
        ...
        if ( meta.isPerformLookup() ) {
          ...
          if ( log.isRowLevel() )
            logRowlevel( BaseMessages.getString( PKG,
                "SynchronizeAfterMerge.Log.ValuesSetForLookup", data.lookupParameterRowMeta.getString( lookupRow ) ) ); //$NON-NLS-1$
          ...
          if ( add == null ) {
            ...
            throw new KettleDatabaseException( BaseMessages.getString( PKG,
                "SynchronizeAfterMerge.Exception.KeyCouldNotFound", data.lookupParameterRowMeta
                    .getString( lookupRow ) ) );
          } else {
            if ( log.isRowLevel() )
              logRowlevel( BaseMessages.getString( PKG,
                  "SynchronizeAfterMerge.Log.FoundRowForUpdate", data.insertRowMeta.getString( row ) ) ); //$NON-NLS-1$
        ...
        if ( operation.equals( data.updateValue ) ) {
          if ( !meta.isPerformLookup() || updateorDelete ) {
            ...
            if ( meta.istablenameInField() ) {
              data.updateStatement =
                  (PreparedStatement) data.preparedStatements.get( data.realSchemaTable + "update" );
          ...
          if ( log.isRowLevel() )
            logRowlevel( BaseMessages
                .getString(
                    PKG,
                    "SynchronizeAfterMerge.Log.SetValuesForDelete", data.deleteParameterRowMeta.getString( deleteRow ), data.inputRowMeta.getString( row ) ) ); //$NON-NLS-1$
  ...
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
    ...
    if ( first ) {
      ...
      // lookup the values!
      if ( log.isDebug() )
        logDebug( BaseMessages.getString( PKG, "SynchronizeAfterMerge.Log.CheckingRow" ) + r.toString() ); //$NON-NLS-1$
      ...
      for ( int i = 0; i < meta.getKeyStream().length; i++ ) {
        ...
        if ( data.keynrs[i] < 0 && // couldn't find field!
          ...
          throw new KettleStepException( BaseMessages.getString( PKG,
              "SynchronizeAfterMerge.Exception.FieldRequired", meta.getKeyStream()[i] ) ); //$NON-NLS-1$ //$NON-NLS-2$
        }
        ...
        if ( data.keynrs2[i] < 0 && // couldn't find field!
          ...
          throw new KettleStepException( BaseMessages.getString( PKG,
              "SynchronizeAfterMerge.Exception.FieldRequired", meta.getKeyStream2()[i] ) ); //$NON-NLS-1$ //$NON-NLS-2$
        }

        if ( log.isDebug() )
          logDebug( BaseMessages.getString( PKG,
              "SynchronizeAfterMerge.Log.FieldHasDataNumbers", meta.getKeyStream()[i] ) + data.keynrs[i] ); //$NON-NLS-1$ //$NON-NLS-2$
      }
      ...
      for ( int i = 0; i < meta.getUpdateLookup().length; i++ ) {
        ...
        {
          throw new KettleStepException( BaseMessages.getString( PKG,
              "SynchronizeAfterMerge.Exception.FieldRequired", meta.getUpdateStream()[i] ) ); //$NON-NLS-1$ //$NON-NLS-2$
        }
        if ( log.isDebug() )
          logDebug( BaseMessages.getString( PKG,
              "SynchronizeAfterMerge.Log.FieldHasDataNumbers", meta.getUpdateStream()[i] ) + data.valuenrs[i] ); //$NON-NLS-1$ //$NON-NLS-2$
      }
    ...
    try {
      ...
      if ( checkFeedback( getLinesRead() ) ) {
        if ( log.isDetailed() )
          logDetailed( BaseMessages.getString( PKG, "SynchronizeAfterMerge.Log.LineNumber" ) + getLinesRead() ); //$NON-NLS-1$
      }
