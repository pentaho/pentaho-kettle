...
public class GetTransStatusServlet extends BaseHttpServlet implements CarteServletInterface {
  ...
  protected void doGet( HttpServletRequest request, HttpServletResponse response )
    ...
    if ( trans != null ) {
      ...
      String logText =
          CentralLogStore.getAppender().getBuffer( trans.getLogChannel().getLogChannelId(), false, startLineNr,
              lastLineNr ).toString();
      ...
      if ( useXML ) {
        ...
        try {
          ...
          if ( ( trans.isFinished() && trans.isRunning() )
            ...
            out.print( "<p>" );
            out.print( "<a href=\"" + convertContextPath( PrepareExecutionTransServlet.CONTEXT_PATH ) + "?name="
                + URLEncoder.encode( transName, "UTF-8" ) + "&id=" + URLEncoder.encode( id, "UTF-8" ) + "\">"
          ...
          for ( int i = 0; i < trans.nrSteps(); i++ ) {
            ...
            if ( ( step.isRunning() ) || step.getStatus() != StepExecutionStatus.STATUS_EMPTY ) {
              ...
              if ( step.isRunning() && !step.isStopped() && !step.isPaused() ) {
                String sniffLink =
                    " <a href=\"" + convertContextPath( SniffStepServlet.CONTEXT_PATH ) + "?trans="
                        + URLEncoder.encode( transName, "UTF-8" ) + "&id=" + URLEncoder.encode( id, "UTF-8" )
          ...
          out.println( "<p>" );
          out.println( "<textarea id=\"translog\" cols=\"120\" rows=\"20\""
