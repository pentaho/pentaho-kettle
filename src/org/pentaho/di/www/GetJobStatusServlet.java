...
public class GetJobStatusServlet extends BaseHttpServlet implements CarteServletInterface {
  ...
  protected void doGet( HttpServletRequest request, HttpServletResponse response )
    ...
    if ( job != null ) {
      ...
      String logText =
          CentralLogStore.getAppender().getBuffer( job.getLogChannel().getLogChannelId(), false, startLineNr,
              lastLineNr ).toString();
      ...
      if ( useXML ) {
        ...
        try {
          ...
          out.print( "<a href=\"" + convertContextPath( GetJobStatusServlet.CONTEXT_PATH ) + "?name="
              + URLEncoder.encode( jobName, "UTF-8" ) + "&xml=y&id=" + URLEncoder.encode( id, "UTF-8" ) + "\">"
      ...
      if ( useXML ) {
        out.println( new WebResult( WebResult.STRING_ERROR, BaseMessages.getString( PKG,
            "StartJobServlet.Log.SpecifiedJobNotFound", jobName, id ) ) );
      } else {
