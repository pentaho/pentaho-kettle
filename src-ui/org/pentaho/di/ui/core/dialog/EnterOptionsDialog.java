...
public class EnterOptionsDialog extends Dialog
{
	...
	private void addGeneralTab()
	{
		...
		wClearCustom.addSelectionListener(new SelectionAdapter()
		{
			public void widgetSelected(SelectionEvent e)
			{
				...
				if (id == SWT.YES)
				{
					...
					ok
							.setMessage(Messages
									.getString("EnterOptionsDialog.ClearCustomParameters.Confirmation"));
					ok.open();
				}
			}
		});
		...
		// updates
		Label updLbl = new Label(wGeneralComp, SWT.RIGHT);
		updLbl.setText(Messages.getString("EnterOptionsDialog.VersionCheck.Label"));
		props.setLook(updLbl);
		FormData updData = new FormData();
		updData.left = new FormAttachment(0, 0);
		updData.top = new FormAttachment(tooltipLbl, margin);
		updData.right = new FormAttachment(middle, -margin);
		updLbl.setLayoutData(updData);
		vcBtn = new Button(wGeneralComp, SWT.CHECK);
		props.setLook(vcBtn);
		vcBtn.setSelection(props.skipVersionCheck());
		FormData updBtnData = new FormData();
		updBtnData.left = new FormAttachment(middle, 0);
		updBtnData.top = new FormAttachment(tooltipLbl, margin);
		updBtnData.right = new FormAttachment(100, 0);
		vcBtn.setLayoutData(updBtnData);
	...
	private void ok()
	{
		...
		props.setSkipVersionCheck(vcBtn.getSelection());
