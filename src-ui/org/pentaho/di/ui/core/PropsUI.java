...
public class PropsUI extends Props
{
	...
	private static final String IGNORE_LISTENERS = "IgnoreListeners";
	...
	public void setDefault()
	{
		...
		if (display != null)
		{
			...
			String prop = BasePropertyHandler.getProperty(
					"Default_UI_Properties_Resource", "org.pentaho.di.ui.core.default"); //$NON-NLS-1$ //$NON-NLS-2$
			try
			{
	...
	{
		LastUsedFile lastUsedFile = new LastUsedFile(fileType, filename, directory, sourceRepository,
				repositoryName);
	...
	public void loadLastUsedFiles()
	{
		...
		for (int i = 0; i < nr; i++)
		{
			String fileType = properties.getProperty(
					"filetype" + (i + 1), LastUsedFile.FILE_TYPE_TRANSFORMATION); // default:
			// transformation
			...
			lastUsedFiles.add(new LastUsedFile(fileType, filename, directory, sourceRepository,
					repositoryName));
		}
	}
	...
	public FontData getDefaultFont()
	{
		...
		// System.out.println("Font default: ["+name+"], size="+size+",
		// style="+style+", default font name = "+def.getName());
	...
	public RGB getBackgroundRGB()
	{
		...
		int g = Const
				.toInt(properties.getProperty(STRING_BACKGROUND_COLOR_G), ConstUI.COLOR_BACKGROUND_GREEN);
	...
	{
		int r = Const.toInt(properties.getProperty(STRING_GRAPH_COLOR_R), ConstUI.COLOR_GRAPH_RED); // default
		// White
	...
	{
		int r = Const.toInt(properties.getProperty(STRING_TAB_COLOR_R), ConstUI.COLOR_TAB_RED); // default
		// White
	...
	{
		String s = properties.getProperty(IGNORE_LISTENERS);
		...
		String props = StringUtils.join(newList.toArray(), ",");
		properties.setProperty(IGNORE_LISTENERS, props);
	}

	public boolean isListenerDisabled(String name)
	{
		String s = properties.getProperty(IGNORE_LISTENERS);
		if (s != null)
		{
