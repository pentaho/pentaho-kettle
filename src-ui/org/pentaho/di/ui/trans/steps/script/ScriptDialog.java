...
public class ScriptDialog extends BaseStepDialog implements StepDialogInterface
{
	...
	private  void addCtab(String cScriptName, String strScript, int iType){
		...
		else wScript.setText(BaseMessages.getString(PKG, "ScriptDialog.ScriptHere.Label")+Const.CR+Const.CR); 
    ...
    */
	    ...
	    for (String strFunction : v) {
	       ...
	       TreeItem itemFunction=null;
			switch(iFunctionType){
				case ScriptAddedFunctions.STRING_FUNCTION: itemFunction = new TreeItem(itemStringFunctionsGroup,SWT.NULL); break;
				case ScriptAddedFunctions.NUMERIC_FUNCTION:itemFunction = new TreeItem(itemNumericFunctionsGroup,SWT.NULL); break;
				case ScriptAddedFunctions.DATE_FUNCTION:itemFunction = new TreeItem(itemDateFunctionsGroup,SWT.NULL); break;
	...
	private void rebuildInputFieldsTree(){
		try{
			String itemName = BaseMessages.getString(PKG, "ScriptDialog.InputFields.Label");
		...
		addDeleteItem.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
		        ...
		        if(tItem!=null){
		        	MessageBox messageBox = new MessageBox(shell, SWT.ICON_QUESTION | SWT.NO | SWT.YES);
		            messageBox.setText(BaseMessages.getString(PKG, "ScriptDialog.DeleteItem.Label"));
		            messageBox.setMessage(BaseMessages.getString(PKG, "ScriptDialog.ConfirmDeleteItem.Label",tItem.getText()));
		            switch(messageBox.open()){
		            	case SWT.YES:
