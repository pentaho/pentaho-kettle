...
public class UserDefinedJavaClassHelp {

	private static Document dom;
	private static Hashtable<String, String> hatFunctionsList;
	
	
	public UserDefinedJavaClassHelp(String strFileName) throws KettleXMLException {
		super();
	...
	public String getSample(String strFunctionName, String strFunctionNameWithArgs){
		...
		for(int i=0;i<nl.getLength();i++){
			if(nl.item(i).getAttributes().getNamedItem("field_name").getNodeValue().equals(strFunctionName)){
	...
	private static void xparseXmlFile(String strFileName) throws KettleXMLException{
        try
        {
            String fileContents = parseFile(strFileName);
            dom = XMLHandler.loadXMLString(fileContents);
        }
        catch(Exception e)
        {
    ...
    private static String parseFile(String strFileName)
    {
        StringBuffer buffer = new StringBuffer();
        try
        {
            InputStream is = UserDefinedJavaClassHelp.class.getResourceAsStream(strFileName);
            int c;
