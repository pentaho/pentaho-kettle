 ...
 * @since 19-jun-2003
 */
public class RepositoriesDialog implements XulEventHandler {
  private static Class<?> PKG = KettleDatabaseRepositoryDialog.class; // for i18n purposes, needed by Translator2!!   $NON-NLS-1$

  private static final String XUL_FILE_TOOLBAR = "ui/repository-login-toolbar.xul"; //$NON-NLS-1$
  ...
  public RepositoriesDialog(Display disp, String toolName, Permission... permissions) {
    ...
    shell.setText(BaseMessages.getString(PKG, "RepositoriesDialog.Dialog.Main.Title")); //$NON-NLS-1$
  ...
  public boolean open() {
    ...
    shell.setText(BaseMessages.getString(PKG, "RepositoriesDialog.Label.SelectRepository")); //$NON-NLS-1$
    ...
    wlKettle.setText(BaseMessages.getString(PKG, "RepositoriesDialog.Label.Welcome", toolName, Const.VERSION)); //$NON-NLS-1$
    ...
    // Repository name
    wRepName = new LabelText(shell, SWT.SINGLE | SWT.LEFT, BaseMessages.getString(PKG, "RepositoriesDialog.Label.RepName"), BaseMessages.getString(PKG, "RepositoriesDialog.Label.RepName"), middle, margin); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    wRepDesc = new LabelText(shell, SWT.SINGLE | SWT.LEFT, BaseMessages.getString(PKG, "RepositoriesDialog.Label.RepDesc"), BaseMessages.getString(PKG, "RepositoriesDialog.Label.RepDesc"), middle, margin); //$NON-NLS-1$//$NON-NLS-2$
    ...
    // Repository type
    wRepTypeName = new LabelText(shell, SWT.SINGLE | SWT.LEFT, BaseMessages.getString(PKG, "RepositoriesDialog.Label.RepType"), BaseMessages.getString(PKG, "RepositoriesDialog.Label.RepType"), middle, margin); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    // Username
    wUsername = new LabelText(shell, BaseMessages.getString(PKG, "RepositoriesDialog.Label.Login"), BaseMessages.getString(PKG, "RepositoriesDialog.Label.Login"), middle, margin); //$NON-NLS-1$//$NON-NLS-2$
    ...
    // Password
    wPassword = new LabelText(shell, BaseMessages.getString(PKG, "RepositoriesDialog.Label.Password"), BaseMessages.getString(PKG, "RepositoriesDialog.Label.Password"), middle, margin); //$NON-NLS-1$//$NON-NLS-2$
    ...
    wShow.setText(BaseMessages.getString(PKG, "RepositoriesDialog.Button.Show")); //$NON-NLS-1$
    wShow.setToolTipText(BaseMessages.getString(PKG, "RepositoriesDialog.Button.Show.ToolTip")); //$NON-NLS-1$
    props.setLook(wShow);
    fdShow = new FormData();
    ...
    wNorep.setText(BaseMessages.getString(PKG, "RepositoriesDialog.Button.NoRepository")); //$NON-NLS-1$
  ...
  public void newRepository() {
    ...
    //
    EnterSelectionDialog selectRepositoryType = new EnterSelectionDialog(shell, names, "Select the repository type", "Select the repository type to create"); //$NON-NLS-1$//$NON-NLS-2$
    ...
    if (choice != null) {
      ...
      try {
        ...
        new ErrorDialog(shell, "Error", "Error creating new repository", e); //$NON-NLS-1$//$NON-NLS-2$
      }
    }
  }
  ...
  public void editRepository() {
    try {
      ...
      new ErrorDialog(shell, "Error", "Error editing repository", e); //$NON-NLS-1$//$NON-NLS-2$
    }
  }
  ...
  public void deleteRepository() {
    ...
    if (index >= 0) {
      ...
      if (repositoryMeta != null) {
        ...
        messageBox.setText(BaseMessages.getString(PKG, "RepositoriesDialog.DelRepo.AreYouSure.Title")); //$NON-NLS-1$
        messageBox.setMessage(BaseMessages.getString(PKG, "RepositoriesDialog.DelRepo.AreYouSure.Message")); //$NON-NLS-1$
  ...
  private void addToolBar() {
    try {
     XulLoader loader = new SwtXulLoader();
      ...
      swtToolBar = (ToolBar) toolbar.getManagedObject();
      swtToolBar.layout(true, true);

    } catch (Throwable t) {
      new ErrorDialog(shell, BaseMessages.getString(PKG, "RepositoriesDialog.Exception.ErrorReadingXULFile.Title"), BaseMessages.getString(PKG, "RepositoriesDialog.Exception.ErrorReadingXULFile.Message", XUL_FILE_TOOLBAR), new Exception(t)); //$NON-NLS-1$ //$NON-NLS-2$
    }
  }

  protected void showSelectedRepository() {

    wRepName.setText(""); //$NON-NLS-1$
    wRepDesc.setText(""); //$NON-NLS-1$
    wRepTypeName.setText(""); //$NON-NLS-1$
    ...
    if (!Const.isEmpty(selection) && selection.length == 1) {
      ...
      if (meta != null) {
        ...
        wRepDesc.setText(Const.NVL(meta.getDescription(), "")); //$NON-NLS-1$
  ...
  private void ok() {
    if (wRepository.getItemCount() != 0) {
      ...
      if (idx >= 0) {
        ...
        mb.setText(BaseMessages.getString(PKG, "RepositoriesDialog.Dialog.PleaseSelectARepsitory.Title")); //$NON-NLS-1$
        mb.open();
