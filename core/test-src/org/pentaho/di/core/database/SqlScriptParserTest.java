...
public class SqlScriptParserTest {
  ...
  public void testSplit() {
    ...
    Assert.assertEquals( Collections.singletonList( "SELECT \"hello\\\"world\" FROM dual" ),
            sqlScriptParser.split( "SELECT \"hello\\\"world\" FROM dual" ) );
    Assert.assertEquals( Collections.singletonList( "CREATE TABLE test1 (col1 STRING) TBLPROPERTIES (\"prop1\" = \"my\\\"value\")" ),
    ...
    Assert.assertEquals( Collections.singletonList( "SELECT 'test\\';SELECT 1" ), sqlScriptParser.split( "SELECT 'test\\';SELECT 1" ) );
    Assert.assertEquals( Arrays.asList( "create table pdi13654 (col1 string) TBLPROPERTIES (\"quoteChar\"=\"\\\"\", \"escapeChar\"=\"\\\\\")", "SELECT 1" ),
  ...
  public void testRemoveComments() {
    ...
    Assert.assertEquals( "SELECT INSTR('/loader/*/*.txt', '/') - INSTR('/loader/*/*.txt', '/') ",
            sqlScriptParser.removeComments( "SELECT INSTR('/loader/*/*.txt', '/') - INSTR('/loader/*/*.txt', '/') " ) );
    Assert.assertEquals( "SELECT  col1, col2, col3 FROM account WHERE name = 'Pentaho'",
            sqlScriptParser.removeComments( "SELECT /* my data*/ col1, col2, col3 FROM account WHERE name = 'Pentaho'" ) );
    Assert.assertEquals( "SELECT /*+ ORACLE hint*/ col1, col2, col3 FROM account WHERE name = 'Pentaho'",
    ...
    Assert.assertEquals( "CREATE TABLE test1 (col1 STRING) TBLPROPERTIES (\"prop1\" = \"my\\\"value\")",
