...
public class VariablesTest {
  ...
  public void testinItializeVariablesFrom() {
    ...
    Mockito.doCallRealMethod().when( variablesMock ).initializeVariablesFrom( Matchers.any( VariableSpace.class ) );
    ...
    } ).when( propertiesMock ).put( Matchers.anyString(), Matchers.anyString() );
    ...
    return new Callable<Boolean>() {
      ...
      public Boolean call() throws Exception {
        for ( int i = 0; i < 300; i++ ) {
          ...
          Assert.assertEquals( variables.environmentSubstitute( "${" + key + "}" ), "value" );
        }
        return true;
      }
    };
