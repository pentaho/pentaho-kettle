...
public class DatabaseFactory implements DatabaseFactoryInterface {
      ...
      if (databaseMeta.isPartitioned()) {
        ...
        for (int i = 0; i < partitioningInformation.length; i++) {
          try {
            ...
            report.append(BaseMessages.getString(PKG, "DatabaseMeta.report.ConnectionWithPartOk",
                databaseMeta.getName(), partitioningInformation[i].getPartitionId())
                + Const.CR);
          } catch (KettleException e) {
            report.append(BaseMessages.getString(PKG, "DatabaseMeta.report.ConnectionWithPartError",
                databaseMeta.getName(), partitioningInformation[i].getPartitionId(), e.toString())
                + Const.CR);
  ...
  private StringBuffer appendConnectionInfo(StringBuffer report, Database db, DatabaseMeta databaseMeta) {
    ...
    if (databaseMeta.getDatabaseInterface() instanceof GenericDatabaseMeta) {
      ...
      String customDriverClass = databaseMeta.getAttributes().getProperty(
          GenericDatabaseMeta.ATRRIBUTE_CUSTOM_DRIVER_CLASS);

      return report.append(BaseMessages.getString(PKG, "GenericDatabaseMeta.report.customUrl"))
          .append(db.environmentSubstitute(customUrl)).append(Const.CR)
          .append(BaseMessages.getString(PKG, "GenericDatabaseMeta.report.customDriverClass"))
          .append(db.environmentSubstitute(customDriverClass)).append(Const.CR);
    }

    return appendConnectionInfo(report, db.environmentSubstitute(databaseMeta.getHostname()),
