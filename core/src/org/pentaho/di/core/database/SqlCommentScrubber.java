 ...
 *  a string representing a SQL query. It respects the notion of a string literal, such that if a comment appears
 *  in a string literal, it is treated as part of the string instead of a comment. Both single-quoted and double-quoted
 *  string literals are supported, including nested quotes (whether the SQL dialect supports them or not).
 * 
 * @author Matt Burgess
 */
public class SqlCommentScrubber {
	...
	/**
	 * This method will remove SQL comments (both multi-line and single-line) from a string representing
	 *  a SQL query. It respects the notion of a string literal, such that if a comment appears in a string 
	 *  literal, it is treated as part of the string instead of a comment. A simple state machine is implemented,
	 *  keeping track of whether the current character is starting, ending, or inside a comment construct. The state
	 *  machine also checks to see if the current character is starting, ending, or inside a single-quoted string 
	 *  literal, as this takes precedence over comment constructs. In other words, comments inside strings are not 
	 *  actually comments, they are part of the string literal.
	 *  
