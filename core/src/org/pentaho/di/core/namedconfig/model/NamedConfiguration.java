...
public class NamedConfiguration extends SharedObjectBase implements Cloneable, XMLInterface, SharedObjectInterface, VariableSpace {
  ...
  private String name;
  @MetaStoreAttribute
  private String type = XML_TAG; // something like hadoop cluster, mongo, vfs browser
  @MetaStoreAttribute
   ...
   */
  public boolean canProvide( Map<String, String[]> required ) {
    if ( required != null ) {
      for ( String groupName : required.keySet() ) {
        ...
        String[] properties = required.get( groupName );
        if ( properties != null ) {
   ...
   * @return true if all of the required properties can be provided
   */
  public boolean hasValuesFor( Map<String, String[]> required ) {
    if ( required != null ) {
      for ( String groupName : required.keySet() ) {
        // every group must be found
        Group group = getGroup( groupName );
        ...
        if ( properties != null ) {
          for ( String property : properties ) {
            // every property must be found and have a value
            if ( !group.containsProperty( property ) || 
                StringUtils.isEmpty( group.getProperty( property ).getPropertyValue() ) ) {
              // if the group does not contain the required property, return false
              return false;
            }
          }
  ...
  public boolean doesConfigurationSatisfy( NamedConfiguration namedConfiguration ) {
    ...
    for ( Group group : namedConfiguration.groups ) {
      String[] groupProperties = new String[group.getProperties().size()];
      ...
      for ( int i = 0; i < groupProperties.length; i++ ) {
        ...
        groupProperties[i] = property.getPropertyName();
      }
    }
