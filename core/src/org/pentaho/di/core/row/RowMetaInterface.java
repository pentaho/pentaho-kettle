 ...
 *  The interface is similar to the JDBC ResultSet interface in that it provides a means of getting data from a row. 
 *  <p>
 *  Inside processRow() a step typically retrieves the structure of incoming rows by 
 *  calling getInputRowMeta(), which is provided by the BaseStep class. The step would 
 *  usually clone this object and pass it to getFields() of its meta class, to reflect 
 *  any changes in row structure caused by the step itself. Now the step has RowMetaInterface 
 *  objects describing both the input and output rows.
 *  <p>
