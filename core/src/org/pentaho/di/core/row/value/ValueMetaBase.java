...
public class ValueMetaBase implements ValueMetaInterface {
  ...
  /**
   * Clones the data. Normally, we don't have to do anything here, but just for
   * arguments and safety, we do a little extra work in case of binary blobs and
   * Date objects. We should write a programmers manual later on to specify in
   * all clarity that
  ...
  public String getCompatibleString(Object object) throws KettleValueException {
    try {
      ...
      case TYPE_NUMBER:
        ...
        case STORAGE_TYPE_INDEXED:
          string = object == null ? null
              : convertNumberToCompatibleString((Double) index[((Integer) object).intValue()]);
          break;
        default:
          throw new KettleValueException(toString() + " : Unknown storage type " + storageType + " specified.");
        }
      ...
      case TYPE_INTEGER:
        ...
        case STORAGE_TYPE_INDEXED:
          string = object == null ? null
              : convertIntegerToCompatibleString((Long) index[((Integer) object).intValue()]);
          break;
        default:
          throw new KettleValueException(toString() + " : Unknown storage type " + storageType + " specified.");
        }
  ...
  public byte[] getBinaryString(Object object) throws KettleValueException {
    ...
    try {
      ...
      case TYPE_DATE:
        ...
        case STORAGE_TYPE_BINARY_STRING:
          return convertStringToBinaryString(convertDateToString((Date) convertBinaryStringToNativeType((byte[]) object)));
        case STORAGE_TYPE_INDEXED:
      ...
      case TYPE_NUMBER:
        ...
        case STORAGE_TYPE_BINARY_STRING:
          return convertStringToBinaryString(convertNumberToString((Double) convertBinaryStringToNativeType((byte[]) object)));
        case STORAGE_TYPE_INDEXED:
      ...
      case TYPE_INTEGER:
        ...
        case STORAGE_TYPE_BINARY_STRING:
          return convertStringToBinaryString(convertIntegerToString((Long) convertBinaryStringToNativeType((byte[]) object)));
        case STORAGE_TYPE_INDEXED:
      ...
      case TYPE_BIGNUMBER:
        ...
        case STORAGE_TYPE_BINARY_STRING:
          return convertStringToBinaryString(convertBigNumberToString((BigDecimal) convertBinaryStringToNativeType((byte[]) object)));
        case STORAGE_TYPE_INDEXED:
      ...
      case TYPE_BOOLEAN:
        ...
        case STORAGE_TYPE_BINARY_STRING:
          return convertStringToBinaryString(convertBooleanToString((Boolean) convertBinaryStringToNativeType((byte[]) object)));
        case STORAGE_TYPE_INDEXED:
          ...
          for (int i = 0; i < index.length; i++) {
            try {
              ...
              case TYPE_BINARY:
                ...
                break;
              default:
                throw new KettleFileException(toString() + " : Unable to serialize indexe storage type for data type "
                    + getType());
              }
            } catch (ClassCastException e) {
              throw new RuntimeException(toString() + " : There was a data type error: the data type of "
        ...
        for (int i = 0; i < index.length; i++) {
          try {
            ...
            case TYPE_BINARY:
              ...
              break;
            default:
              throw new IOException(toString() + " : Unable to serialize indexe storage type to XML for data type "
                  + getType());
            }
          } catch (ClassCastException e) {
            throw new RuntimeException(toString() + " : There was a data type error: the data type of "
  ...
  /**
   * Convert a data XML node to an Object that corresponds to the metadata. This
   * is basically String to Object conversion that is being done.
   * 
   * @param node
  ...
  /**
   * TODO: change Desc to Code all over the place. Make sure we can localise
   * this stuff later on.
   * 
   * @param type
   *          the type
   ...
   * @return The integer storage type of the given String.
   *         (ValueMetaInterface.STORAGE_TYPE_...) or -1 if the storage type
   *         code not be found.
   */
  ...
  /**
   * Determine if an object is null. This is the case if data==null or if it's
   * an empty string.
   * 
   * @param data
   *          the object to test
  ...
  /*
   * Compare 2 binary strings, one byte at a time.<br> This algorithm is very
   * fast but most likely wrong as well.<br>
   * 
   * @param one The first binary string to compare with
   * 
   * @param two the second binary string to compare to
   * 
   * @return -1 if <i>one</i> is smaller than <i>two</i>, 0 is both byte arrays
   * are identical and 1 if <i>one</i> is larger than <i>two</i> protected int
   * compareBinaryStrings(byte[] one, byte[] two) {
   * 
   * for (int i=0;i<one.length;i++) { if (i>=two.length) return 1; // larger if
   * (one[i]>two[i]) return 1; // larger if (one[i]<two[i]) return -1; //
   * smaller } if (one.length>two.length) return 1; // larger if
   * (one.length>two.length) return -11; // smaller return 0; }
   */
   ...
   * @param data2
   *          the second value
   * @return 0 if the values are equal, -1 if data1 is smaller than data2 and +1
   *         if it's larger.
   * @throws KettleValueException
  ...
  /**
   * @return the bigNumberFormatting flag : true if BigNumbers of formatted as
   *         well
   */
   ...
   *          the bigNumberFormatting flag to set : true if BigNumbers of
   *          formatted as well
   */
