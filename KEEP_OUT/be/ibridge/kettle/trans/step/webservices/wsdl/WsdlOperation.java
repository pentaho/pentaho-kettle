...
public final class WsdlOperation implements java.io.Serializable {
    ...
    private void loadFaults(Operation op) {
        ...
        for (Iterator itr = faultMap.values().iterator(); itr.hasNext();) {
            _faults.add((Fault) itr.next());
        }
    }
    ...
    private void loadParameters(Operation op) {
        ...
        if (input != null) {
            ...
            for (Iterator itr = inParts.iterator(); itr.hasNext();) {
                _params.add((Part) itr.next(), true);
            }
        }
        ...
        if (output != null) {
            ...
            for (Iterator itr = outParts.iterator(); itr.hasNext();) {
                _oneway = false;
                _params.add((Part) itr.next(), false);
            }
        }
    }
}
