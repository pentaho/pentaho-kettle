 ...
 */
public final class WsdlOpParameterList extends ArrayList
{

    private static final long serialVersionUID = 1L;
    private final Operation _operation;
    private final WsdlTypes _wsdlTypes;
    private final HashSet _headerNames;
    ...
    /**
     * Was there a 'return type' parameter in this list?  If so return its
     * XML type.
     *
    ...
    protected boolean add(Part p, boolean requestPart) {
        ...
        List params = getParameter(p, requestPart);
        for (Iterator itr = params.iterator(); itr.hasNext();) {

            WsdlOpParameter op = (WsdlOpParameter) itr.next();
     ...
     */
    private List getParameter(Part part, boolean requesPart) {
    ...
    /**
     * Add an response param to the parameter list. Some rules for determining
     * if the request param is the return value for the operation:
     * <p/>
     * <ol>
     * <li>If the operation has 'parameterOrder' set:</li>
     * <ol>
     * <li>If the response parameter is not in the operation's parameterOrder
     * attribute, then it represents the return value of the call.
     * If there is no such part, then the method does not return a value.</li>
     * b) If the response parameter is found in the parameterOrder list,
     * add it as an OUT mode parameter.</li>
     * </ol>
     * <li>If the operation does not have 'parameterOrder' set:</li>
     * <ol>
     * <li>If there is a single part in the output message that is not also
     * in the input message it is mapped to the return type of the method.</li>
     * <li>If there is more than one part in the output message that is not in
     * the input message they are all mapped as out arguments and the return
     ...
     */
    private List resolvePartElement(Part p) {
