...
public class GetJobStatusServlet extends BaseHttpServlet implements CartePluginInterface {
  ...
  @VisibleForTesting
  TransJobStatusCache cache = TransJobStatusCache.getInstance();
  ...
  /**
   * <div id="mindtouch"> <h1>/kettle/jobStatus</h1> <a name="GET"></a> <h2>GET</h2> <p>Retrieves status of the
   * specified job. Status is returned as HTML or XML output depending on the input parameters. Status contains
   * information about last execution of the job.</p>
   * <p>
   * <p><b>Example Request:</b><br /> <pre function="syntax.xml"> GET /kettle/jobStatus/?name=dummy_job&xml=Y </pre>
   * <p>
   * </p> <h3>Parameters</h3> <table class="pentaho-table"> <tbody> <tr> <th>name</th> <th>description</th>
   * <th>type</th> </tr> <tr> <td>name</td> <td>Name of the job to be used for status generation.</td> <td>query</td>
   * </tr> <tr> <td>xml</td> <td>Boolean flag which defines output format <code>Y</code> forces XML output to be
   * generated. HTML is returned otherwise.</td> <td>boolean, optional</td> </tr> <tr> <td>id</td> <td>Carte id of the
   * job to be used for status generation.</td> <td>query, optional</td> </tr> <tr> <td>from</td> <td>Start line number
   * of the execution log to be included into response.</td> <td>integer, optional</td> </tr> </tbody> </table>
   * <p>
   * <h3>Response Body</h3>
   * <p>
   * <table class="pentaho-table"> <tbody> <tr> <td align="right">element:</td> <td>(custom)</td> </tr> <tr> <td
   * align="right">media types:</td> <td>text/xml, text/html</td> </tr> </tbody> </table> <p>Response XML or HTML
   * response containing details about the job specified. If an error occurs during method invocation
   * <code>result</code> field of the response will contain <code>ERROR</code> status.</p>
   * <p>
   * <p><b>Example Response:</b></p> <pre function="syntax.xml"> <?xml version="1.0" encoding="UTF-8"?> <jobstatus>
   * <jobname>dummy_job</jobname> <id>a4d54106-25db-41c5-b9f8-73afd42766a6</id> <status_desc>Finished</status_desc>
   * <error_desc/> <logging_string>&#x3c;&#x21;&#x5b;CDATA&#x5b;
   * H4sIAAAAAAAAADMyMDTRNzTUNzRXMDC3MjS2MjJQ0FVIKc3NrYzPyk8CsoNLEotKFPLTFEDc1IrU5NKSzPw8Xi4j4nRm5qUrpOaVFFUqRLuE
   * &#x2b;vpGxhKj0y0zL7M4IzUFYieybgWNotTi0pwS2&#x2b;iSotLUWE1iTPNCdrhCGtRsXi4AOMIbLPwAAAA&#x3d;&#x5d;&#x5d;&#x3e;
   * </logging_string>
   * <first_log_line_nr>0</first_log_line_nr> <last_log_line_nr>20</last_log_line_nr> <result>
   * <lines_input>0</lines_input> <lines_output>0</lines_output> <lines_read>0</lines_read>
   * <lines_written>0</lines_written> <lines_updated>0</lines_updated> <lines_rejected>0</lines_rejected>
   * <lines_deleted>0</lines_deleted> <nr_errors>0</nr_errors> <nr_files_retrieved>0</nr_files_retrieved>
   * <entry_nr>0</entry_nr> <result>Y</result> <exit_status>0</exit_status> <is_stopped>N</is_stopped> <log_channel_id/>
   * <log_text>null</log_text> <result-file></result-file> <result-rows></result-rows> </result> </jobstatus> </pre>
   * <p>
   * <h3>Status Codes</h3> <table class="pentaho-table"> <tbody> <tr> <th>code</th> <th>description</th> </tr> <tr>
   * <td>200</td> <td>Request was processed.</td> </tr> <tr> <td>500</td> <td>Internal server error occurs during
   * request processing.</td> </tr> </tbody> </table> </div>
   */
    ...
    if ( job != null ) {
      ...
      if ( useXML ) {
        try {
          ...
          if ( finishedOrStopped && ( data = cache.get( id, logId ) ) != null ) {
