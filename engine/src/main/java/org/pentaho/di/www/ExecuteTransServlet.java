...
public class ExecuteTransServlet extends BaseHttpServlet implements CartePluginInterface {
  ...
  protected Map<StepMeta, List<RowMetaAndData>> executeTrans( Trans trans, Map<StepMeta, List<RowMetaAndData>> previewDataMap ) throws KettleException {
    trans.prepareExecution( null );
    Map<StepMeta, List<RowMetaAndData>> previewDataMapUpdated = null;
    if ( isPostCall ) {
      previewDataMapUpdated = capturePreviewData( trans, trans.getTransMeta().getSteps(), previewDataMap );
    }
    trans.startThreads();
  ...
  void executeTransformation( TransMeta transMeta, HttpServletRequest request, HttpServletResponse response, boolean isPostCall ) throws KettleException, IOException {
    ...
    try {
      ...
      //
      Map<StepMeta, List<RowMetaAndData>> previewDataMap = executeTrans( trans, new HashMap<>() );
  ...
  public Map<StepMeta, List<RowMetaAndData>> capturePreviewData( final Trans trans, List<StepMeta> stepMetas, Map<StepMeta, List<RowMetaAndData>> previewDataMap ) {
