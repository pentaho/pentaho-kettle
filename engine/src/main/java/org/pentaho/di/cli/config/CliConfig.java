...
import org.pentaho.di.pan.CliFileSupport;
...
public class CliConfig {
  ...
  public static final String TOKEN_STORE_BACKEND_OS = "os";
  ...
  public synchronized String getTokenStoreBackend() {
    ...
    if ( TOKEN_STORE_BACKEND_FILE.equals( value ) || TOKEN_STORE_BACKEND_OS.equals( value ) ) {
      return value;
    }
  ...
  public synchronized boolean setTokenStoreBackend( String backend ) {
    if ( !TOKEN_STORE_BACKEND_OS.equals( backend ) && !TOKEN_STORE_BACKEND_FILE.equals( backend ) ) {
  ...
  private boolean saveCliConfig() {
    try {
      CliFileSupport.ensureParentDirectoryExists( configFile );
  ...
  private void setRestrictiveFilePermissions( File file ) {
    CliFileSupport.applyOwnerOnlyPermissions( file, message( "CliConfig.TargetDescription" ) );
  }
