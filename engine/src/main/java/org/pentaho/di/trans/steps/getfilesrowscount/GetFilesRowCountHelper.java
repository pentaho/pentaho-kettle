...
public class GetFilesRowCountHelper extends BaseStepHelper {
   ...
   *
   * <p>Initializes the helper by calling the parent {@link BaseStepHelper} constructor
   * which sets up logging infrastructure and common helper functionality.</p>
   */
  public GetFilesRowCountHelper() {
    super();
  }
   ...
   *
   * @param transMeta   The transformation metadata containing the step configuration
   * @param queryParams A map of query parameters, must contain "stepName" key
   * @return A JSON object containing the file list and operation status
   * @throws ClassCastException   if the step is not a GetFilesRowsCountMeta instance
  ...
  public JSONObject getFilesAction( TransMeta transMeta, Map<String, String> queryParams ) {
    JSONObject response = new JSONObject();
    GetFilesRowsCountMeta getFilesRowsCountMeta =
      (GetFilesRowsCountMeta) transMeta.findStep( queryParams.get( "stepName" ) ).getStepMetaInterface();
