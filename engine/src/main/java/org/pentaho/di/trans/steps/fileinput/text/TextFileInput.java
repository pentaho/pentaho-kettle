...
public class TextFileInput extends BaseFileInputStep<TextFileInputMeta, TextFileInputData> implements StepInterface, CsvInputAwareStepUtil {
  ...
  public JSONObject getFieldsAction( Map<String, String> queryParams ) throws KettleException, JsonProcessingException {
    ...
    if ( TextFileInputMeta.FILE_TYPE_CSV == tfii.getFileTypeNr() ) {
      jsonArray = populateMeta( queryParams );
    } else {
  ...
  public JSONArray populateMeta( Map<String, String> queryParams ) throws KettleException, JsonProcessingException {
    ...
    while ( !errorFound && line != null && ( linenr <= samples || samples == 0 ) ) { // TODO && !monitor.isCanceled()
      // TODO monitor.subTask( BaseMessages.getString( PKG, "TextFileCSVImportProgressDialog.Task.ScanningLine", "" + linenr ) );

      if ( samples > 0 ) {
        // TODO monitor.worked( 1 );
      }

      if ( log.isDebug() ) {
        // TODO debug = "convert line #" + linenr + " to row";
      }
    ...
    // TODO monitor.worked( 1 );
    // TODO monitor.setTaskName( BaseMessages.getString( PKG, "TextFileCSVImportProgressDialog.Task.AnalyzingResults" ) );
