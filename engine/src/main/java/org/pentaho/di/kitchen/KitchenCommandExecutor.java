        ...
        try {
            ...
            if (!Utils.isEmpty(params.getRepoName()) || !Utils.isEmpty(params.getLocalFile())) {
                ...
                if (!Utils.isEmpty(params.getRepoName()) && !isEnabled(params.getBlockRepoConns())) {
                    ...
                    // In case we use a repository...
                    // some commands are to load a Trans from the repo; others are merely to print
                    // some repo-related information
                    RepositoryMeta repositoryMeta = loadRepositoryConnection(params.getRepoName(),
                            "Kitchen.Log.LoadingRep", "Kitchen.Error.NoRepDefinied",
                    ...
                    boolean useBrowserAuth = Utils.isEmpty(params.getRepoPassword())
                            || isEnabled(params.getBrowserAuth());
                    ...
                    repository = establishRepositoryConnectionWithBrowserAuth(repositoryMeta, params.getRepoUsername(),
        ...
        if (!Utils.isEmpty(params.getExportRepo())) {
            ...
            try {
                ...
                TopLevelResource topLevelResource = ResourceUtil.serializeResourceExportInterface(getBowl(),
                        repository == null ? DefaultBowl.getInstance()
                                : repository.getBowl(),
    ...
    protected void printRepositories(RepositoriesMeta repositoriesMeta) {
        ...
        if (repositoriesMeta != null) {
            ...
            for (int i = 0; i < repositoriesMeta.nrRepositories(); i++) {
                ...
                System.out.println(
                        "#" + (i + 1) + " : " + rinfo.getName() + " [" + rinfo.getDescription() + "]  id="
                                + rinfo.getId());
            }
        }
    }
