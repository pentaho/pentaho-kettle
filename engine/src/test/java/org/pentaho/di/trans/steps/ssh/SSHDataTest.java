...
public class SSHDataTest {
  ...
  public void setup() throws Exception {
    ...
    when( SSHData.getFile( fileObject ) ).thenReturn( file );
  ...
  public void testOpenConnectionUseKey_2() throws Exception {
    ...
    when( connection.authenticateWithPublicKey( username, file, passPhrase ) ).thenReturn( true );
    SSHData.OpenConnection( server, port, username, null, true, keyFilePath,
      passPhrase, 0, variableSpace, null, 0, null, null );
    verify( connection ).connect();
    verify( connection ).authenticateWithPublicKey( username, file, passPhrase );
  }
