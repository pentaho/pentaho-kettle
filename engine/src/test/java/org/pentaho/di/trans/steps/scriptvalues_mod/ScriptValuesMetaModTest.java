...
public class ScriptValuesMetaModTest implements InitializerInterface<StepMetaInterface> {
  ...
  public void testGetType() {
    final ScriptValuesMetaMod meta = new ScriptValuesMetaMod();

    Whitebox.setInternalState( meta, "type", new int[] {} );
    Whitebox.setInternalState( meta, "typeStr", new String[] { "String", "Integer" } );

    Assert.assertEquals( 2, meta.getType().length );
    ...
    Whitebox.setInternalState( meta, "type", new int[] { -1 } );

    Assert.assertEquals( 2, meta.getType().length );
    Assert.assertEquals( ValueMetaInterface.TYPE_STRING, meta.getType()[ 0 ] );
    Assert.assertEquals( ValueMetaInterface.TYPE_INTEGER, meta.getType()[ 1 ] );

    Whitebox.setInternalState( meta, "type", new int[] { ValueMetaInterface.TYPE_NUMBER } );

    Assert.assertEquals( 1, meta.getType().length );
    Assert.assertEquals( ValueMetaInterface.TYPE_NUMBER, meta.getType()[ 0 ] );
  ...
  public void testExtend() {
    ...
    Whitebox.setInternalState( meta, "typeStr", new String[] { "String", "Integer", "Number" } );
  ...
  public void testClone() {
    ...
    ScriptValuesMetaMod clone = (ScriptValuesMetaMod) meta.clone();
    validateExtended( clone );
  }
}
