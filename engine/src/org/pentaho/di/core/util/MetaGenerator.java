    ...
    for ( TypeFieldDefinition field : fields ) {
      switch ( field.getType() ) {
        ...
        case ValueMeta.TYPE_INTEGER:
          code.append(
              "    "
                  + field.getMemberName() + " = Const.toInt(XMLHandler.getTagValue(stepnode, \"" + field.getFieldName()
                  + "\"), -1);" ).append( Const.CR );
          break;
        default:
          break;
    ...
    for ( TypeFieldDefinition field : fields ) {
      switch ( field.getType() ) {
        ...
        case ValueMeta.TYPE_INTEGER:
          code.append(
              "    "
                  + field.getMemberName() + " = (int) rep.getStepAttributeInteger(id_step, \"" + field.getFieldName()
                  + "\");" ).append( Const.CR );
          break;
        default:
          break;
