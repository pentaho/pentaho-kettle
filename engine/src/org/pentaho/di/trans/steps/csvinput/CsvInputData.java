...
public class CsvInputData extends BaseStepData implements StepDataInterface {
	...
	private void resizeByteBufferArray() {
		...
		// No need to add extra len for buffer
	...
	private int readBufferFromFile() throws IOException {
		...
		// Since this method doesn't get called every other character, I'm sure we can
		// spend a bit of time here without
	...
	/**
	 * Check to see if the buffer size is large enough given the data.endBuffer
	 * pointer.<br>
	 * Resize the buffer if there is not enough room.
	 *
	 * @return false if everything is OK, true if there is a problem and we should
	 *         stop.
	 ...
	 * If there is not enough room in the buffer to go there, resize the byte buffer
	 * and read more data.<br>
	 * if there is no more data to read and if the endBuffer pointer has reached the
	 * end of the byte buffer, we return true.<br>
	 *
	...
	/**
	 * This method should be used very carefully. Moving pointer without increasing
	 * number of written bytes can lead to data corruption.
	 */
