...
public class CsvInput extends BaseStep implements StepInterface {
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
		...
		if (first) {
			...
			// Then skip to the appropriate file and location in the file to start
			// reading...
		...
		// If we are running in parallel, make sure we don't read too much in this step
		// copy...
		//
		if (data.parallel) {
		...
		try {
			...
			if (outputRowData == null) {
				...
				if (checkFeedback(getLinesInput())) {
					if (log.isBasic()) {
						logBasic(
								BaseMessages.getString(PKG, "CsvInput.Log.LineNumber", Long.toString(getLinesInput())));
					}
				}
			}
			...
			if (getStepMeta().isDoingErrorHandling()) {
				...
				putError(data.outputRowMeta, e.getRowData(), e.getCauses().size(), errorDescriptions.toString(),
						errorFields.toString(), "CSVINPUT001");
			} else {
		...
		try {
			...
			for (int i = 0; i < data.fileSizes.size(); i++) {
				...
				if (data.startPosition >= totalFileSize && data.startPosition < totalFileSize + size) {
					...
					// remember where we started to read to allow us to know that we have to skip
					// the header row in the next files
					// (if any)
					//
					data.startFilenr = i;
			...
			if (data.filenames.length > 0) {
				logBasic(BaseMessages.getString(PKG, "CsvInput.Log.ParallelFileNrAndPositionFeedback",
						data.filenames[data.filenr], Long.toString(data.fileSizes.get(data.filenr)),
			...
			if (firstRow) {
				...
				if (index < 0) {
					throw new KettleException(
							BaseMessages.getString(PKG, "CsvInput.Exception.FilenameFieldNotFound", filenameField));
				}
			}
	...
	private boolean openNextFile() throws KettleException {
		try {
			...
			if (!(fileObject instanceof LocalFile)) {
				// We can only use NIO on local files at the moment, so that's what we limit
				// ourselves to.
				//
			...
			// If we are running in parallel and we need to skip bytes in the first file,
			// let's do so here.
			//
			if (data.parallel) {
			...
			// - If you're running in parallel, if a header row is checked, if you're at the
			// beginning of a file
			//
	...
	/**
	 * We need to skip row only if a line, that we are currently on is read by the
	 * previous step <b>partly</b>. In other words, we DON'T skip a line if we are
	 * just beginning to read it from the first symbol. We have to do some work for
	 * this: read last byte from the previous step and make sure that it is a new
	 * line byte. But it's not enough. There could be a situation, where new line is
	 * indicated by '\r\n' construction. And if we are <b>between</b> this
	 * construction, we want to skip last '\n', and don't want to include it in our
	 * line.
	 *
	 * So, we DON'T skip line only if the previous char is new line indicator AND we
	 * are not between '\r\n'.
	 *
	 */
	private boolean needToSkipRow() {
		try {
			...
			if (data.newLineFound()) {
				// don't increase bytes read for this step, as it is actually content of another
				// step
				// and we are reading this just for evaluation.
				data.moveEndBufferPointer(false);
				// now we are at the first char of our thread.
				// there is still a situation we want to avoid: when there is a windows style
				// "/r/n", and we are between two
				// of this chars. In this case we need to skip a line. Otherwise we don't skip
				// it.
	 ...
	 *
	 * @param skipRow          if row should be skipped: header row or part of row
	 *                         in case of parallel read
	 * @param ignoreEnclosures if enclosures should be ignored, i.e. in case of we
	 *                         need to skip part of the row during parallel read
	...
	private Object[] readOneRow(boolean skipRow, boolean ignoreEnclosures) throws KettleException {
		...
		try {
			...
			while (!newLineFound && outputIndex < meta.getInputFields().length) {
				...
				// Also skip over the enclosures ("), it is NOT taking into account escaped
				// enclosures.
				// Later we can add an option for having escaped or double enclosures in the
				// file. <sigh>
				//
				...
				while (!delimiterFound && !newLineFound && !endOfBuffer) {
					...
					if (data.delimiterFound()) {
						...
						// If the flag is enable we skip newline checking except for the last field in
						// the row.
						...
						if (fieldFirstBytePosition == enclosurePosition) {
							...
							do {
								...
								if (!keepGoing) {
									...
									// If this character is also an enclosure, we can consider the enclosure
									// "escaped".
				...
				if (!skipRow) {
					if (meta.isLazyConversionActive()) {
						...
						// The convert object uses binary storage as such we just have to ask the native
						// type from it.
				...
				// PDI-8187: Before we increment, we should check to see if the while condition
				// is about to fail.
				// this will prevent the endBuffer from being incremented twice (once by this
				// block and once in the
				// do-while loop below) and possibly skipping a newline character. This can
...
//					}
			...
			// If not, we need to skip the remaining items on the line until the next
			// newline...
			//
			if (!newLineFound && !data.resizeBufferIfNeeded()) {
				do {
					...
					// TODO: if we're using quoting we might be dealing with a very dirty file with
					// quoted newlines in trailing
					// fields. (imagine that)
		...
		if (super.init(smi, sdi)) {
			...
			try {
				...
				if (Utils.isEmpty(meta.getEnclosure())) {
					...
					data.enclosure = data.encodingType.getBytes(environmentSubstitute(meta.getEnclosure()),
							realEncoding);
				}
