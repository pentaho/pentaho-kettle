...
public class CsvInputMeta extends BaseStepMeta
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			...
			lazyConversionActive = "Y"
					.equalsIgnoreCase(XMLHandler.getTagValue(stepnode, getXmlCode("LAZY_CONVERSION")));
	...
	public String getXML() {
		...
		for (int i = 0; i < inputFields.length; i++) {
			...
			retval.append("        ").append(XMLHandler.addTagValue(getXmlCode("FIELD_TYPE"),
	...
	public void readRep(Repository rep, IMetaStore metaStore, ObjectId id_step, List<DatabaseMeta> databases)
		...
		try {
			...
			newlinePossibleInFields = rep.getStepAttributeBoolean(id_step, 0, getRepCode("NEWLINE_POSSIBLE"),
					!runningInParallel);
			...
			for (int i = 0; i < nrfields; i++) {
				...
				inputFields[i].setType(ValueMetaFactory
						.getIdForValueMeta(rep.getStepAttributeString(id_step, i, getRepCode("FIELD_TYPE"))));
				...
				inputFields[i]
						.setPrecision((int) rep.getStepAttributeInteger(id_step, i, getRepCode("FIELD_PRECISION")));
				inputFields[i].setTrimType(ValueMetaString
						.getTrimTypeByCode(rep.getStepAttributeString(id_step, i, getRepCode("FIELD_TRIM_TYPE"))));
			}
		} catch (Exception e) {
	...
	public void saveRep(Repository rep, IMetaStore metaStore, ObjectId id_transformation, ObjectId id_step)
		...
		try {
			...
			for (int i = 0; i < inputFields.length; i++) {
				...
				rep.saveStepAttribute(id_transformation, id_step, i, getRepCode("FIELD_CURRENCY"),
						field.getCurrencySymbol());
				rep.saveStepAttribute(id_transformation, id_step, i, getRepCode("FIELD_DECIMAL"),
				...
				rep.saveStepAttribute(id_transformation, id_step, i, getRepCode("FIELD_PRECISION"),
						field.getPrecision());
				rep.saveStepAttribute(id_transformation, id_step, i, getRepCode("FIELD_TRIM_TYPE"),
						ValueMetaString.getTrimTypeCode(field.getTrimType()));
			}
		} catch (Exception e) {
	...
	public void getFields(RowMetaInterface rowMeta, String origin, RowMetaInterface[] info, StepMeta nextStep,
		...
		try {
			...
			for (int i = 0; i < inputFields.length; i++) {
				...
				// transformation.
				//
				ValueMetaInterface storageMetadata = ValueMetaFactory.cloneValueMeta(valueMeta,
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta, StepMeta stepMeta, RowMetaInterface prev,
		...
		if (prev == null || prev.size() == 0) {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
					BaseMessages.getString(PKG, "CsvInputMeta.CheckResult.NotReceivingFields"), stepMeta);
			remarks.add(cr);
		} else {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG, "CsvInputMeta.CheckResult.StepRecevingData", prev.size() + ""),
					stepMeta);
			remarks.add(cr);
		}
		...
		if (input.length > 0) {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG, "CsvInputMeta.CheckResult.StepRecevingData2"), stepMeta);
			remarks.add(cr);
		} else {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
					BaseMessages.getString(PKG, "CsvInputMeta.CheckResult.NoInputReceivedFromOtherSteps"), stepMeta);
			remarks.add(cr);
		}
	}
	...
	/**
	 * @param lazyConversionActive true if lazy conversion is to be turned on:
	 *                             conversions are delayed as long as possible,
		...
		for (StepInjectionMetaEntry entry : metadata) {
			...
			if (entry.getValueType() != ValueMetaInterface.TYPE_NONE) {
				...
				if (attr.getKey().equals("FIELDS")) {
					...
					// Each list contains a single CSV input field definition (one line in the
					// dialog)
					//
	...
	/**
	 * Describe the metadata attributes that can be injected into this step metadata
	 * object.
	 *
	 * @throws KettleException
	 */
