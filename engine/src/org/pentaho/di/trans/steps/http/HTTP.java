...
import org.pentaho.di.core.util.ApacheHttpClientFactory;
...
public class HTTP extends BaseStep implements StepInterface {
  ...
  private Object[] callHttpService( RowMetaInterface rowMeta, Object[] rowData ) throws KettleException {
    ApacheHttpClientFactory connectionManager = ApacheHttpClientFactory.getInstance();
    // Prepare HTTP get
    CloseableHttpClient httpClient;

    int inputParametersQuantity = 6;

    if ( data.realConnectionTimeout <= -1 ) {
      inputParametersQuantity = 5;
    }
    if ( data.realSocketTimeout <= -1 ) {
      inputParametersQuantity = 4;
    }
    if ( Utils.isEmpty( data.realHttpLogin ) ) {
      inputParametersQuantity = 2;
    }
    if ( Utils.isEmpty( data.realProxyHost ) ) {
      inputParametersQuantity = 0;
    }

    switch ( inputParametersQuantity ) {
      case 0: httpClient = connectionManager.createHttpClient();
        break;
      case 2: httpClient =
        connectionManager.createHttpClient( data.realProxyHost, data.realProxyPort );
        break;
      case 4: httpClient = connectionManager
        .createHttpClient( data.realHttpLogin, data.realHttpPassword, data.realProxyHost,
          data.realProxyPort );
        break;
      case 5: httpClient = connectionManager
        .createHttpClient( data.realSocketTimeout, data.realHttpLogin, data.realHttpPassword, data.realProxyHost,
          data.realProxyPort );
        break;
      case 6: httpClient = connectionManager
        .createHttpClient( data.realConnectionTimeout, data.realSocketTimeout, data.realHttpLogin,
          data.realHttpPassword, data.realProxyHost, data.realProxyPort );
        break;
      default: httpClient = connectionManager.createHttpClient();
        break;
    }
