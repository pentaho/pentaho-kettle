 ...
 * @since 4-apr-2003
 */
public class OldTextFileInput extends BaseStep implements StepInterface {
  ...
  private OldTextFileInputMeta meta;

  private OldTextFileInputData data;

  private long lineNumberInFile;

  public OldTextFileInput( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta,
      Trans trans ) {
  ...
  public static final String getLine( LogChannelInterface log, InputStreamReader reader, int formatNr,
    ...
    OldEncodingType type = OldEncodingType.guessEncodingType( reader.getEncoding() );
    return getLine( log, reader, type, formatNr, line );
  }

  public static final String getLine( LogChannelInterface log, InputStreamReader reader, OldEncodingType encodingType,
      int formatNr, StringBuilder line ) throws KettleFileException {
    int c = 0;
    line.setLength( 0 );
    try {
      switch ( formatNr ) {
        case OldTextFileInputMeta.FILE_FORMAT_DOS:
          while ( c >= 0 ) {
            c = reader.read();
          ...
          break;
        case OldTextFileInputMeta.FILE_FORMAT_UNIX:
          while ( c >= 0 ) {
            c = reader.read();
          ...
          break;
        case OldTextFileInputMeta.FILE_FORMAT_MIXED:
  ...
  @Deprecated
  public static final String[] guessStringsFromLine( LogChannelInterface log, String line, OldTextFileInputMeta inf,
  ...
  public static final String[] guessStringsFromLine( VariableSpace space, LogChannelInterface log, String line,
      OldTextFileInputMeta inf, String delimiter, String enclosure, String escapeCharacter ) throws KettleException {
  ...
  /**
   * @deprecated Use {@link #convertLineToRow(LogChannelInterface, OldTextFileLine,
   ...
   */
  @Deprecated
  public static final Object[] convertLineToRow( LogChannelInterface log, OldTextFileLine textFileLine,
  ...
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
    data = (OldTextFileInputData) sdi;
    meta = (OldTextFileInputMeta) smi;
    Object[] r = null;
    boolean retval = true;
     ...
     */
    OldTextFileLine textLine = data.lineBuffer.get( 0 );
    incrementLinesInput();
    lineNumberInFile++;
  ...
  private boolean tryToReadLine( boolean applyFilter ) throws KettleFileException {
    ...
    if ( line != null ) {
      ...
      if ( applyFilter ) {
        ...
        if ( filterOK ) {
          data.lineBuffer.add( new OldTextFileLine( line, lineNumberInFile, data.file ) ); // Store it in the
          // line buffer...
        } else {
          return false;
        }
        ...
        if ( !meta.noEmptyLines() || line.length() != 0 ) {
          data.lineBuffer.add( new OldTextFileLine( line, lineNumberInFile, data.file ) ); // Store it in the line
                                                                                        // buffer...
        }
      }
    } else {
  ...
  public boolean init( StepMetaInterface smi, StepDataInterface sdi ) {
    ...
    if ( super.init( smi, sdi ) ) {
      ...
      data.filterProcessor = new OldTextFileFilterProcessor( meta.getFilter() );
