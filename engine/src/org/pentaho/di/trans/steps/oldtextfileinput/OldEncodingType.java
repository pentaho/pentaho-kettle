...
public enum OldEncodingType {
   ...
   * @param bom
   */
  private OldEncodingType( int length, int bom, int carriageReturnChar, int lineFeedChar ) {
    this.length = length;
    this.bom = bom;
  ...
  public static OldEncodingType guessEncodingType( String encoding ) {

    OldEncodingType encodingType;

    if ( Const.isEmpty( encoding ) ) {
      encodingType = OldEncodingType.SINGLE;
    } else if ( encoding.startsWith( "UnicodeBig" ) || encoding.equals( "UTF-16BE" ) ) {
      encodingType = OldEncodingType.DOUBLE_BIG_ENDIAN;
    } else if ( encoding.startsWith( "UnicodeLittle" ) || encoding.equals( "UTF-16LE" ) ) {
      encodingType = OldEncodingType.DOUBLE_LITTLE_ENDIAN;
    } else if ( encoding.equals( "UTF-16" ) ) {
      encodingType = OldEncodingType.DOUBLE_BIG_ENDIAN; // The default, no BOM
    } else {
      encodingType = OldEncodingType.SINGLE;
    }
