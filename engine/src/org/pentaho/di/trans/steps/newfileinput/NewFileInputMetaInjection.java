...
public class NewFileInputMetaInjection implements StepMetaInjectionInterface {
  ...
  public NewFileInputMetaInjection( NewFileInputMeta meta ) {
    this.meta = meta;
  }
  ...
  public void injectStepMetadataEntries( List<StepInjectionMetaEntry> all ) throws KettleException {
    ...
    List<NewFileInputField> fields = new ArrayList<NewFileInputField>();
    List<NewFileFilter> filters = new ArrayList<NewFileFilter>();
        ...
        case FILTERS:
          for ( StepInjectionMetaEntry lookField : lookFields.getDetails() ) {
            ...
            if ( fieldEntry == Entry.FILTER ) {
              NewFileFilter filterLine = new NewFileFilter();
    ...
    //
    if ( fields.size() > 0 ) {
      meta.inputFiles.inputFields= fields.toArray( new NewFileInputField[fields.size()] ) ;
    }
    if ( fileLines.size() > 0 ) {
    ...
    if ( filters.size() > 0 ) {
      meta.setFilter( filters.toArray( new NewFileFilter[filters.size()] ) );
    }
  ...
  public List<StepInjectionMetaEntry> extractStepMetadataEntries() throws KettleException {
    ...
    if ( !Const.isEmpty( meta.inputFiles.inputFields ) ) {
      for ( NewFileInputField inputField : meta.inputFiles.inputFields ) {
        StepInjectionMetaEntry fieldEntry = getEntry( Entry.FIELD );
        fieldsEntry.getDetails().add( fieldEntry );
  ...
  public NewFileInputMeta getMeta() {
    return meta;
  }
}
