 ...
 */
public class NewFileInputReader implements IBaseInputReader {
  private static Class<?> PKG = NewFileInputReader.class; // for i18n purposes, needed by Translator2!!

  private static final int BUFFER_SIZE_INPUT_STREAM = 500;

  private final IBaseInputStepControl step;
  private final NewFileInputMeta meta;
  private final NewFileInputData data;
  ...
  public NewFileInputReader( IBaseInputStepControl step, NewFileInputMeta meta, NewFileInputData data, FileObject file,
  ...
  protected void readInitial() throws Exception {
    ...
    bufferSize +=
        meta.content.layoutPaged ? meta.content.nrLinesPerPage * ( Math.max( 0, meta.content.nrWraps ) + 1 ) : Math.max( 0, meta
            .content.nrWraps ); // it helps when we have wrapped input w/o header
    ...
    if ( meta.content.layoutPaged ) {
      for ( int i = 0; i < meta.content.nrLinesDocHeader; i++ ) {
        // Just skip these...
        NewFileInputUtils.getLine( log, isr, data.encodingType, data.fileFormatType, data.lineStringBuilder ); // header
                                                                                                               // and
        // footer: not
        // wrapped
        lineNumberInFile++;
  ...
  public boolean readRow() throws KettleException {
     ...
     */
    NewFileLine textLine = data.lineBuffer.get( 0 );
    ...
    if ( meta.content.layoutPaged ) {
      ...
      if ( !data.doneWithHeader && data.pageLinesRead == 0 ) // We are reading header lines
      {
        ...
        if ( data.pageLinesRead < meta.content.nrLinesPerPage ) {
          ...
          r =
              NewFileInputUtils.convertLineToRow( log, textLine, meta, data.currentPassThruFieldsRow,
  ...
  protected boolean tryToReadLine( boolean applyFilter ) throws KettleFileException {
    String line;
    line = NewFileInputUtils.getLine( log, isr, data.encodingType, data.fileFormatType, data.lineStringBuilder );
    if ( line != null ) {
      ...
      if ( applyFilter ) {
        ...
        if ( filterOK ) {
          data.lineBuffer.add( new NewFileLine( line, lineNumberInFile, data.file ) ); // Store it in the
          // line buffer...
        } else {
          return false;
        }
        ...
        if ( !meta.content.noEmptyLines || line.length() != 0 ) {
          data.lineBuffer.add( new NewFileLine( line, lineNumberInFile, data.file ) ); // Store it in the line
                                                                                       // buffer...
        }
      }
    } else {
