 ...
 */
public class NewFileInputUtils {
  private static Class<?> PKG = NewFileInputUtils.class; // for i18n purposes, needed by Translator2!!

  public static final String[] guessStringsFromLine( VariableSpace space, LogChannelInterface log, String line,
      NewFileInputMeta inf, String delimiter, String enclosure, String escapeCharacter ) throws KettleException {
    ...
    try {
      ...
      if ( inf.content.fileType.equalsIgnoreCase( "CSV" ) ) {
        ...
        for ( int i = 0; i < inf.inputFiles.inputFields.length; i++ ) {
          NewFileInputField field = inf.inputFiles.inputFields[i];
  ...
  public static final String getLine( LogChannelInterface log, InputStreamReader reader, EncodingType encodingType,
    ...
    try {
      switch ( formatNr ) {
        case NewFileInputMeta.FILE_FORMAT_DOS:
          while ( c >= 0 ) {
            c = reader.read();
          ...
          break;
        case NewFileInputMeta.FILE_FORMAT_UNIX:
          while ( c >= 0 ) {
            c = reader.read();
          ...
          break;
        case NewFileInputMeta.FILE_FORMAT_MIXED:
  ...
  public static final Object[] convertLineToRow( LogChannelInterface log, NewFileLine NewFileLine,
      NewFileInputMeta info, Object[] passThruFields, int nrPassThruFields, RowMetaInterface outputRowMeta,
      ...
      BaseInputStepMeta.AdditionalOutputFields additionalOutputFIelds, String shortFilename, String path,
      boolean hidden, Date modificationDateTime, String uri, String rooturi, String extension, long size )
        throws KettleException {
    if ( NewFileLine == null || NewFileLine.line == null ) {
      return null;
    }
    ...
    try {
      ...
      String[] strings = convertLineToStrings( log, NewFileLine.line, info, delimiter, enclosure, escapeCharacter );
      int shiftFields = ( passThruFields == null ? 0 : nrPassThruFields );
      for ( fieldnr = 0; fieldnr < nrfields; fieldnr++ ) {
        NewFileInputField f = info.inputFiles.inputFields[fieldnr];
          ...
          try {
            ...
            if ( info.errorHandling.errorIgnored ) {
              ...
              if ( errorHandler != null ) {
                errorHandler.handleLineError( NewFileLine.lineNumber, AbstractFileErrorHandler.NO_PARTS );
              }
  ...
  public static final String[] convertLineToStrings( LogChannelInterface log, String line, NewFileInputMeta inf,
          ...
          if ( len_encl > 0 && line.substring( from, from + len_encl ).equalsIgnoreCase( enclosure ) ) {
            ...
            boolean is_enclosure =
                len_encl > 0 && p + len_encl < length && line.substring( p, p + len_encl ).equalsIgnoreCase(
                    enclosure );
            boolean is_escape =
                len_esc > 0 && p + len_esc < length && line.substring( p, p + len_esc ).equalsIgnoreCase( inf
            ...
            while ( ( !is_enclosure || enclosure_after ) && p < line.length() ) {
              ...
              is_enclosure =
                  len_encl > 0 && p + len_encl < length && line.substring( p, p + len_encl ).equals( enclosure );
              is_escape =
                  len_esc > 0 && p + len_esc < length && line.substring( p, p + len_esc ).equals( inf
                      .content.escapeCharacter );
