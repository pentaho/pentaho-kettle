...
public class WebService extends BaseStep implements StepInterface {
  ...
  private HttpClient getHttpClient( HttpClientContext context ) {
    ...
    String host = environmentSubstitute( meta.getProxyHost() );
    if ( StringUtils.isBlank( host ) ) {
      parametersQuantity = 4;
    }
    switch ( parametersQuantity ) {
      case 0:
      default:
        httpClient = connectionManager.createHttpClient();
        break;
      case 2:
        httpClient = connectionManager.createHttpClient( login, environmentSubstitute( meta.getHttpPassword() ) );
        break;
      case 4:
        int port = Const.toInt( environmentSubstitute( meta.getProxyPort() ), 8080 );
        httpClient = connectionManager
          .createHttpClient( login, environmentSubstitute( meta.getHttpPassword() ), host,
            port
          );
