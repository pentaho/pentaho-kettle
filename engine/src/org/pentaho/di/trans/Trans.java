          ...
          for (int c = 0; c < nrCopies; c++) {
            ...
            switch (transMeta.getTransformationType()) {
              case Normal:
                ...
                // amounts of rows.
                //
                Boolean batchingRowSet = ValueMeta.convertStringToBoolean(System
                    .getProperty(Const.KETTLE_BATCHING_ROWSET));
  ...
  *	
  ...
  protected void addStepPerformanceSnapShot() {
    ...
    if (transMeta.isCapturingStepPerformanceSnapShots() && !pausedAndNotEmpty && !stoppedAndNotEmpty) {
      ...
      for (int i = 0; i < steps.size(); i++) {
        ...
        StepPerformanceSnapShot snapShot = new StepPerformanceSnapShot(seqNr, getBatchId(), new Date(), getName(),
            stepMeta.getName(), step.getCopy(), step.getLinesRead(), step.getLinesWritten(), step.getLinesInput(),
            step.getLinesOutput(), step.getLinesUpdated(), step.getLinesRejected(), step.getErrors());
    ...
    try {
      if (logConnection != null) {
        ...
        if (transLogTable.isBatchIdUsed()) {
          Long id_batch = logConnection.getNextBatchId(transLogTableDatabaseConnection, logSchema, logTable,
              transLogTable.getKeyField().getFieldName());
          ...
          TimerTask timerTask = new TimerTask() {
            public void run() {
              try {
                ...
                log.logError(BaseMessages.getString(PKG, "Trans.Exception.UnableToPerformIntervalPerformanceLogging"),
                    e);
                // Also stop the show...
                //
                errors.incrementAndGet();
  ...
  protected void writeStepLogInformation() throws KettleException {
    ...
    try {
      ...
      throw new KettleException(BaseMessages.getString(PKG, "Trans.Exception.UnableToWriteStepInformationToLogTable"),
          e);
    } finally {
      if (!db.isAutoCommit())
        db.commit(true);
          ...
          try {
            ...
            if (result.getNrErrors() > 0) {
              ...
              try {
                ...
                log.logBasic(BaseMessages.getString(PKG, "Trans.Exception.TransactionsCommittedOnConnection",
                    database.toString()));
              } catch (Exception e) {
                throw new KettleDatabaseException(BaseMessages.getString(PKG,
            ...
            log.logError(
                BaseMessages.getString(PKG, "Trans.Exception.ErrorHandlingTransformationTransaction",
                    database.toString()), e);
  ...
  public static final TransSplitter executeClustered(final TransMeta transMeta,
    ...
    //
    executionConfiguration.getVariables()
        .put(Const.INTERNAL_VARIABLE_CLUSTER_RUN_ID, transSplitter.getClusteredRunId());
    ...
    try {
      ...
      if (masterSteps.size() > 0) // If there is something that needs to be done on the master...
      {
        ...
        if (executionConfiguration.isClusterPosting()) {
          ...
          String masterReply = masterServer.sendXML(transConfiguration.getXML(), AddTransServlet.CONTEXT_PATH
              + "/?xml=Y");
          ...
          Runnable runnable = new Runnable() {
            public void run() {
              try {
                ...
                String slaveReply = slaves[index].sendXML(transConfiguration.getXML(), AddTransServlet.CONTEXT_PATH
                    + "/?xml=Y");
      ...
      if (executionConfiguration.isClusterPosting()) {
        if (executionConfiguration.isClusterPreparing()) {
          ...
          if (masterSteps.size() > 0) // If there is something that needs to be done on the master...
          {
            ...
            String masterReply = masterServer.execService(PrepareExecutionTransServlet.CONTEXT_PATH + "/?name="
                + URLEncoder.encode(master.getName(), "UTF-8") + "&id=" + URLEncoder.encode(carteObjectId, "UTF-8")
                + "&xml=Y");
            WebResult webResult = WebResult.fromXMLString(masterReply);
            if (!webResult.getResult().equalsIgnoreCase(WebResult.STRING_OK)) {
              throw new KettleException(
        ...
        if (executionConfiguration.isClusterStarting()) {
          ...
          {
            String carteObjectId = carteObjectMap.get(master);
            String masterReply = masterServer.execService(StartExecutionTransServlet.CONTEXT_PATH + "/?name="
                + URLEncoder.encode(master.getName(), "UTF-8") + "&id=" + URLEncoder.encode(carteObjectId, "UTF-8")
                + "&xml=Y");
            WebResult webResult = WebResult.fromXMLString(masterReply);
            if (!webResult.getResult().equalsIgnoreCase(WebResult.STRING_OK)) {
    ...
    while (!allFinished && errors == 0 && (parentJob == null || !parentJob.isStopped())) {
      ...
      if ((parentJob != null && parentJob.isStopped()) || errors != 0) {
        ...
        try {
          ...
          errors += 1;
          log.logError("Unable to contact master server '" + masterServer.getName() + "' to stop the master : "
              + e.toString());
        }
      }
   ...
   */
  public static void cleanupSlaveServer(TransSplitter transSplitter, SlaveServer slaveServer, TransMeta slaveTransMeta)
      throws KettleException {
  ...
  public static String sendToSlaveServer(TransMeta transMeta, TransExecutionConfiguration executionConfiguration,
    ...
    try {
      ...
      if (executionConfiguration.isPassingExport()) {
        ...
        //
        String result = slaveServer.sendExport(topLevelResource.getArchiveName(), AddExportServlet.TYPE_TRANS,
    ...
    try {
      ...
      if (!WebResult.STRING_OK.equals(webResult.getResult())) {
        log.logError(transName,
            "Unable to run clean-up on remote transformation '" + transName + "' : " + webResult.getMessage());
        errors += 1;
      }
    } catch (Exception e) {
      errors += 1;
