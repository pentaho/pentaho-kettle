...
public class JobPainter extends BasePainter {
  ...
  public JobPainter( GCInterface gc, JobMeta jobMeta, Point area, ScrollBarInterface hori, ScrollBarInterface vert,
      JobHopMeta candidate, Point drop_candidate, Rectangle selrect, List<AreaOwner> areaOwners,
      List<JobEntryCopy> mouseOverEntries, int iconsize, int linewidth, int gridsize, int shadowSize,
      boolean antiAliasing, String noteFontName, int noteFontHeight ) {
  ...
  private void drawJobElements() {
    ...
    if ( candidate != null ) {
      ...
      if ( startHopEntry != null && endHopLocation != null ) {
        ...
        drawArrow( start.x, start.y, end.x, end.y, theta, calcArrowLength(), 1.2, null, startHopEntry,
        ...
        drawArrow( start.x, start.y, end.x, end.y + iconsize / 2, theta, calcArrowLength(), 1.2, null,
  ...
  protected void drawJobEntryCopy( JobEntryCopy jobEntryCopy ) {
    ...
    if ( activeJobEntries != null && activeJobEntries.contains( jobEntryCopy ) ) {
      ...
      areaOwners.add( new AreaOwner( AreaType.JOB_ENTRY_BUSY, iconX, iconY, iconsize, iconsize, offset, subject,
          jobEntryCopy ) );
    } else {
      gc.setForeground( EColor.BLACK );
    }
    ...
    if ( jobEntryResult != null ) {
      ...
      if ( jobEntryResult.isCheckpoint() ) {
        ...
        areaOwners.add( new AreaOwner( AreaType.JOB_ENTRY_RESULT_CHECKPOINT, iconX, iconY, iconsize, iconsize, offset,
            jobEntryCopy, jobEntryResult ) );
      } else {
        if ( result.getResult() ) {
          gc.drawImage( EImage.TRUE, iconX, iconY );
          areaOwners.add( new AreaOwner( AreaType.JOB_ENTRY_RESULT_SUCCESS, iconX, iconY, iconsize, iconsize, offset,
              jobEntryCopy, jobEntryResult ) );
        } else {
          gc.drawImage( EImage.FALSE, x + iconsize, y - 5 );
          areaOwners.add( new AreaOwner( AreaType.JOB_ENTRY_RESULT_FAILURE, iconX, iconY, iconsize, iconsize, offset,
              jobEntryCopy, jobEntryResult ) );
        }
      }
    }
      ...
      for ( int i = 0; i < miniIcons.length; i++ ) {
        ...
        switch ( i ) {
          case 0: // INPUT
            ...
            areaOwners.add( new AreaOwner( AreaType.JOB_ENTRY_MINI_ICON_INPUT, xIcon, yIcon, bounds.x, bounds.y,
                offset, jobMeta, jobEntryCopy ) );
            break;
          case 1: // EDIT
            enabled = true;
            areaOwners.add( new AreaOwner( AreaType.JOB_ENTRY_MINI_ICON_EDIT, xIcon, yIcon, bounds.x, bounds.y, offset,
                jobMeta, jobEntryCopy ) );
            break;
          case 2: // Job entry context menu
            enabled = true;
            areaOwners.add( new AreaOwner( AreaType.JOB_ENTRY_MINI_ICON_CONTEXT, xIcon, yIcon, bounds.x, bounds.y,
                offset, jobMeta, jobEntryCopy ) );
            break;
          case 3: // OUTPUT
            enabled = true;
            areaOwners.add( new AreaOwner( AreaType.JOB_ENTRY_MINI_ICON_OUTPUT, xIcon, yIcon, bounds.x, bounds.y,
                offset, jobMeta, jobEntryCopy ) );
            break;
          default:
            break;
  ...
  private void drawArrow( int[] line, JobHopMeta jobHop, Object startObject, Object endObject ) {
    ...
    drawArrow( screen_from.x, screen_from.y, screen_to.x, screen_to.y, theta, calcArrowLength(), -1, jobHop,
        startObject, endObject );
  }

  private void drawArrow( int x1, int y1, int x2, int y2, double theta, int size, double factor, JobHopMeta jobHop,
    ...
    if ( jobHop != null ) {
      ...
      if ( jobHop.getFromEntry().isLaunchingInParallel() ) {
        ...
        gc.drawImage( hopsIcon, mx, my );
        if ( !shadow ) {
          areaOwners.add( new AreaOwner( AreaType.JOB_HOP_PARALLEL_ICON, mx, my, bounds.x, bounds.y, offset, subject,
              jobHop ) );
        }
      }
      ...
      try {
        ExtensionPointHandler.callExtensionPoint( LogChannel.GENERAL, KettleExtensionPoint.JobPainterArrow.id,
            extension );
      } catch ( Exception e ) {
