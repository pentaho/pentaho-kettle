...
public class KettleDatabaseRepository extends KettleDatabaseRepositoryBase implements Repository {
   ...
   *           in case there is a general unexpected error or if we're already
   *           connected
   */
  ...
  /**
   * Create a new directory, possibly by creating several sub-directies of / at
   * the same time.
   * 
  ...
  public ObjectId[] getDatabaseIDs(boolean includeDeleted) throws KettleException {
    return connectionDelegate.getIDs("SELECT " + quote(KettleDatabaseRepository.FIELD_DATABASE_ID_DATABASE) + " FROM " + quoteTable(KettleDatabaseRepository.TABLE_R_DATABASE) + " ORDER BY "
  ...
  public synchronized String[] getTransformationsUsingDatabase(ObjectId id_database) throws KettleException {
    ObjectId[] transIds = connectionDelegate.getIDs("SELECT DISTINCT " + quote(FIELD_STEP_DATABASE_ID_TRANSFORMATION) + " FROM " + quoteTable(KettleDatabaseRepository.TABLE_R_STEP_DATABASE) + " WHERE " + quote(FIELD_STEP_DATABASE_ID_DATABASE)
        + " = ?", id_database);
  ...
  public ObjectId[] getJobEntryIDs(ObjectId id_job) throws KettleException {
    return connectionDelegate.getIDs("SELECT " + quote(KettleDatabaseRepository.FIELD_JOBENTRY_ID_JOBENTRY) + " FROM " + quoteTable(KettleDatabaseRepository.TABLE_R_JOBENTRY) + " WHERE " + quote(KettleDatabaseRepository.FIELD_JOBENTRY_ID_JOB)
        + " = ?", id_job);
  }
  ...
  /**
   * Set this directory during import to signal that job entries like Trans and
   ...
   * @param upgrade
   *          True if you want to upgrade the repository, false if you want to
   *          create it.
   * @param statements
