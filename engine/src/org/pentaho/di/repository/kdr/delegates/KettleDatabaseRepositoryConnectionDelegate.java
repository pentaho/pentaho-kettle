...
public class KettleDatabaseRepositoryConnectionDelegate extends KettleDatabaseRepositoryBaseDelegate {
  ...
  protected void verifyVersion() throws KettleException {
    ...
    try {
      lastUpgrade = callRead( new Callable<RowMetaAndData>() {
        @Override public RowMetaAndData call() throws Exception {
          return KettleDatabaseRepositoryConnectionDelegate.this.database.getOneRow( "SELECT "
            + KettleDatabaseRepositoryConnectionDelegate.this
            ...
            + " ORDER BY " + KettleDatabaseRepositoryConnectionDelegate.this
      ...
      try {
        ...
        callRead( new Callable<Object>() {
          @Override public Object call() throws Exception {
            return KettleDatabaseRepositoryConnectionDelegate.this.database.getOneRow( "SELECT * FROM " + userTable );
          }
        } );
    ...
    if ( majorVersion == 3 && minorVersion == 0 ) {
      ...
      RowMetaInterface tableFields = callRead( new Callable<RowMetaInterface>() {
        @Override public RowMetaInterface call() throws Exception {
          return KettleDatabaseRepositoryConnectionDelegate.this.database.getTableFields( tableName );
        }
      } );
  ...
  public synchronized void fillStepAttributesBuffer( ObjectId id_transformation ) throws KettleException {
    ...
    final RowMetaAndData parameter = getParameterMetaData( id_transformation );
    stepAttributesBuffer = callRead( new Callable<List<Object[]>>() {
      @Override public List<Object[]> call() throws Exception {
        return KettleDatabaseRepositoryConnectionDelegate.this.database.getRows(
          KettleDatabaseRepositoryConnectionDelegate.this.database
            .openQuery( ps, parameter.getRowMeta(), parameter.getData() ), -1, null );
      }
    } );
  ...
  public synchronized void setLookupTransAttribute() throws KettleException {
    String sql = "SELECT " + quote( KettleDatabaseRepository.FIELD_TRANS_ATTRIBUTE_VALUE_STR )
  ...
  public synchronized void setLookupJobAttribute() throws KettleException {
    String sql = "SELECT " + quote( KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_VALUE_STR ) + ", "
    ...
    if ( stepAttributesBuffer != null ) {
      ...
      final String sql =
        "SELECT COUNT(*) FROM "
          ...
          + quote( KettleDatabaseRepository.FIELD_STEP_ATTRIBUTE_CODE ) + " = ?";
      final RowMetaAndData table = new RowMetaAndData();
      table.addValue( new ValueMetaInteger(
        KettleDatabaseRepository.FIELD_STEP_ATTRIBUTE_ID_STEP ), id_step );
      table.addValue( new ValueMetaString(
        KettleDatabaseRepository.FIELD_STEP_ATTRIBUTE_CODE ), code );
      RowMetaAndData r = callRead( new Callable<RowMetaAndData>() {
        @Override public RowMetaAndData call() throws Exception {
          return KettleDatabaseRepositoryConnectionDelegate.this.database
            .getOneRow( sql, table.getRowMeta(), table.getData() );
        }
      } );
  ...
  public synchronized int countNrTransAttributes( ObjectId id_transformation, String code ) throws KettleException {
    final String sql = "SELECT COUNT(*) FROM " + databaseMeta
  ...
  public synchronized List<Object[]> getTransAttributes( ObjectId id_transformation, String code, long nr )
    throws KettleException {
    final String sql = "SELECT *" + " FROM " + databaseMeta
      .getQuotedSchemaTableCombination( null, KettleDatabaseRepository.TABLE_R_TRANS_ATTRIBUTE )
      + " WHERE " + quote( KettleDatabaseRepository.FIELD_TRANS_ATTRIBUTE_ID_TRANSFORMATION ) + " = ? AND "
      + quote( KettleDatabaseRepository.FIELD_TRANS_ATTRIBUTE_CODE ) + " = ? AND "
    ...
    return callRead( new Callable<List<Object[]>>() {
      @Override public List<Object[]> call() throws Exception {
        return KettleDatabaseRepositoryConnectionDelegate.this.database
          .getRows( sql, table.getRowMeta(), table.getData(), ResultSet.FETCH_FORWARD, false, 0, null );
      }
    } );
  }
  ...
  private synchronized LongObjectId getNextTableID( final String tablename, final String idfield )
    throws KettleException {
    LongObjectId retval = null;
    RowMetaAndData r = callRead( new Callable<RowMetaAndData>() {
      @Override public RowMetaAndData call() throws Exception {
        return KettleDatabaseRepositoryConnectionDelegate.this.database
          .getOneRow( "SELECT MAX(" + idfield + ") FROM " + tablename );
      }
    } );
    if ( r != null ) {
  ...
  public synchronized int countNrJobEntryAttributes( ObjectId id_jobentry, String code ) throws KettleException {
    final String sql = "SELECT COUNT(*) FROM " + databaseMeta.getQuotedSchemaTableCombination(
      null, KettleDatabaseRepository.TABLE_R_JOBENTRY_ATTRIBUTE ) + " WHERE "
      + quote( KettleDatabaseRepository.FIELD_JOBENTRY_ATTRIBUTE_ID_JOBENTRY ) + " = ? AND "
  ...
  public synchronized List<Object[]> getJobEntryAttributesWithPrefix( ObjectId jobId, ObjectId jobEntryId,
    ...
    final String sql = "SELECT *" + " FROM " + databaseMeta.getQuotedSchemaTableCombination(
      null, KettleDatabaseRepository.TABLE_R_JOBENTRY_ATTRIBUTE ) + " WHERE "
  ...
  public synchronized List<RepositoryElementMetaInterface> getRepositoryObjects( String tableName,
    ...
    try {
      ...
      final RowMetaAndData directoryIdRow = getParameterMetaData( id_directory );
  ...
  public ObjectId[] getIDs( String sql, ObjectId... objectId ) throws KettleException {
    ...
    //
    final RowMetaInterface parameterMeta = new RowMeta();
    final Object[] parameterData = new Object[ objectId.length ];
  ...
  public synchronized LongObjectId getIDWithValue( String tablename, String idfield, String lookupfield,
    ...
    try {
      RowMetaAndData result = getOneRow( "SELECT " + idfield + " FROM " + tablename + " WHERE "
        + lookupfield + " = ?", par.getRowMeta(), par.getData() );
  ...
  public Map<String, LongObjectId> getValueToIdMap( String tablename, String idfield, String lookupfield )
    throws KettleException {
    final String sql = new StringBuilder( "SELECT " ).append( lookupfield ).append( ", " ).append( idfield )
      .append( " FROM " ).append( tablename ).toString();
    Map<String, LongObjectId> result = new HashMap<String, LongObjectId>();
    for ( Object[] row : callRead( new Callable<List<Object[]>>() {
      @Override public List<Object[]> call() throws Exception {
        return KettleDatabaseRepositoryConnectionDelegate.this.database
          .getRows( sql, new RowMeta(), new Object[] {}, ResultSet.FETCH_FORWARD, false, -1, null );
      }
    } ) ) {
  ...
  public LongObjectId[] getIDsWithValues( String tablename, String idfield, String lookupfield,
                                          final String[] values ) throws KettleException {
    final String sql = createIdsWithValuesQuery( tablename, idfield, lookupfield, values.length );
    final RowMeta params = new RowMeta();
    for ( int i = 0; i < values.length; i++ ) {
      ValueMetaInterface value = new ValueMetaString( Integer.toString( i ) );
      params.addValueMeta( value );
    }
    List<Object[]> rows = callRead( new Callable<List<Object[]>>() {
      @Override public List<Object[]> call() throws Exception {
        return KettleDatabaseRepositoryConnectionDelegate.this.database
          .getRows( sql, params, values, ResultSet.FETCH_FORWARD, false, -1, null );
      }
    } );
  ...
  public synchronized ObjectId getIDWithValue( String tablename, String idfield, String lookupfield, String value,
    ...
    RowMetaAndData result = getOneRow( "SELECT " + idfield + " FROM " + tablename + " WHERE "
      + lookupfield + " = ? AND " + lookupkey + " = ?", par.getRowMeta(), par.getData() );
    if ( result != null && result.getRowMeta() != null && result.getData() != null && result.isNumeric( 0 ) ) {
      return new LongObjectId( result.getInteger( 0, 0 ) );
    }
    return null;
  ...
  public RowMetaAndData getOneRow( String schemaAndTable, String keyfield, ObjectId id ) throws KettleException {
    ...
    final Object[] parameterData = new Object[] { id != null ? Long.parseLong( id.getId() ) : null, };
    try {
      return callRead( new Callable<RowMetaAndData>() {
        @Override public RowMetaAndData call() throws Exception {
  ...
  public RowMetaAndData getOneRow( final String sql ) throws KettleDatabaseException {
    return callRead( new Callable<RowMetaAndData>() {
      @Override public RowMetaAndData call() throws Exception {
        return KettleDatabaseRepositoryConnectionDelegate.this.database.getOneRow( sql );
      }
    } );
  }

  public RowMetaAndData getOneRow( final String sql, final RowMetaInterface rowMeta, final Object[] rowData )
    throws KettleDatabaseException {
    return callRead( new Callable<RowMetaAndData>() {
      @Override public RowMetaAndData call() throws Exception {
        return KettleDatabaseRepositoryConnectionDelegate.this.database.getOneRow( sql, rowMeta, rowData );
      }
    } );
  }
  ...
  public List<Object[]> getRows( final String sql, final int limit ) throws KettleDatabaseException {
    return callRead( new Callable<List<Object[]>>() {
      @Override public List<Object[]> call() throws Exception {
        return KettleDatabaseRepositoryConnectionDelegate.this.database.getRows( sql, limit );
      }
    } );
  }
  ...
  public Collection<RowMetaAndData> getDatabaseAttributes( ObjectId id_database ) throws KettleDatabaseException,
    ...
    final RowMetaInterface parameterMeta = new RowMeta();
    parameterMeta.addValueMeta( new ValueMetaInteger( "id" ) );
    final Object[] parameterData = new Object[] { ( (LongObjectId) id_database ).longValue(), };
    final List<RowMetaAndData> attrs = new ArrayList<RowMetaAndData>();
