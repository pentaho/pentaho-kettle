...
public class JobLogTest {
  ...
  public void testJobStartLogNegative() throws IOException, KettleException, SQLException, URISyntaxException, InterruptedException {
    ...
    Assert.assertTrue("trackers count more than one", trackers.size()>1);
  }
  ...
  public void testJobStartLogPositive() throws UnknownParamException, KettleXMLException, URISyntaxException, IOException {
    ...
    long actual = trackers.get(5).getJobEntryResult().getResult().getNrErrors();
   ...
   * (at attempt to write success log record (RROR: value too long for type 
  ...
  public void testJobEndLogNegative() throws UnknownParamException, KettleXMLException, URISyntaxException, IOException{
    ...
    List<JobTracker> trackers = tracker.getJobTrackers();
      ...
      //for this case we have 6 trackers messages,
      Assert.assertEquals(trackers.size(), 6);
    
      //last one should have error status
      //(error at the end of execution)
      
      //TODO error for transformation is shown?
      long actual = trackers.get(5).getJobEntryResult().getResult().getNrErrors();
      Assert.assertTrue("Last step generate error?", actual>0);
  }
   ...
   */
  @Test
  public void testJobLogCaller() throws UnknownParamException, KettleXMLException, URISyntaxException, IOException{
   ...
   * 
   * JobTracker + don't follow the DUMMY path.
   *  
  ...
  public void testJobLogTransCaller2Negative() throws UnknownParamException, KettleXMLException, URISyntaxException, IOException{
    ...
    JobTracker track = trackers.get(4);
    //this is for internal transaction fail: 
