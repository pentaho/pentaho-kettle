 ...
 */
public class NewFileInputMetaLoadSaveTest {

  private TransStepLoadSaveTester<NewFileInputMeta> tester;
  ...
  public void setUp() throws Exception {
    ...
    typeValidators.put( NewFileFilter[].class.getCanonicalName(), new ArrayLoadSaveValidator<NewFileFilter>( new NewFileFilterValidator() ) );
    typeValidators.put( NewFileInputField[].class.getCanonicalName(), new ArrayLoadSaveValidator<NewFileInputField>( new NewFileInputFieldValidator() ) );

    assertTrue( !commonAttributes.isEmpty() || !( xmlAttributes.isEmpty() || repoAttributes.isEmpty() ) );

    tester =
        new TransStepLoadSaveTester<NewFileInputMeta>( NewFileInputMeta.class, commonAttributes, xmlAttributes,
  ...
  private static class NewFileInputFieldValidator implements FieldLoadSaveValidator<NewFileInputField> {
    @Override public NewFileInputField getTestObject() {
      return new NewFileInputField( UUID.randomUUID().toString(), new Random().nextInt(), new Random().nextInt() );
    }

    @Override
    public boolean validateTestObject( NewFileInputField testObject, Object actual ) {
      if ( !( actual instanceof NewFileInputField ) ) {
        return false;
      }

      NewFileInputField another = (NewFileInputField) actual;
  ...
  private static class NewFileFilterValidator implements FieldLoadSaveValidator<NewFileFilter> {
    @Override public NewFileFilter getTestObject() {
      NewFileFilter fileFilter = new NewFileFilter();
    ...
    @Override public boolean validateTestObject( NewFileFilter testObject, Object actual ) {
      if ( !( actual instanceof NewFileFilter ) ) {
        return false;
      }

      NewFileFilter another = (NewFileFilter) actual;
      return new EqualsBuilder()
