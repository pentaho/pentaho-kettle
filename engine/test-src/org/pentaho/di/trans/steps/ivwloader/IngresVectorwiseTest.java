...
public class IngresVectorwiseTest {
  ...
  private StepMockHelper<IngresVectorwiseLoaderMeta, IngresVectorwiseLoaderData> stepMockHelper;
  private String[] fieldStream = new String[] { "Number data", "Styring data" };
  ...
  public void setUp() throws Exception {
    ...
    stepMockHelper.redirectLog( new FileOutputStream( "/tmp/log.txt" ), LogLevel.ROWLEVEL );
  ...
  public void testGuiErrors() {
    try {
      ...
      assertTrue( step.getLinesOutput() == 0 );
      assertTrue( step.getLinesRead() == r );
      assertTrue( step.getLinesWritten() == r );
      assertTrue( step.getLinesRejected() == 1 );
      assertTrue( step.getErrors() == 1 );
  ...
  public void testGuiErrorsWithErrorsAllowed() {
    try {
      ...
      ( (IngresVectorwiseLoader) step ).vwLoadMonitorThread.join();
      assertTrue( step.getLinesOutput() == r - 1 );
      assertTrue( step.getLinesRead() == r );
      assertTrue( step.getLinesWritten() == r );
      assertTrue( step.getLinesRejected() == 1 );
      assertTrue( step.getErrors() == 0 );
    } catch ( KettleException e ) {
      fail( e.getMessage() );
    } catch ( InterruptedException e ) {
      e.printStackTrace();
  ...
  public void testGuiSuccess() {
    try {
      ...
      ( (IngresVectorwiseLoader) step ).vwLoadMonitorThread.join();
      assertTrue( step.getLinesOutput() == r );
      assertTrue( step.getLinesRead() == r );
      assertTrue( step.getLinesWritten() == r );
      assertTrue( step.getLinesRejected() == 0 );
      assertTrue( step.getErrors() == 0 );
    } catch ( KettleException e ) {
      fail( e.getMessage() );
    } catch ( InterruptedException e ) {
  ...
  private BaseStep doOutput( List<Object[]> rows, String maxErrorsNumber ) throws KettleException {
    ...
    ivwLoader.getInputRowSets().add( stepMockHelper.getMockInputRowSet( rows ) );
