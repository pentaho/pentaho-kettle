...
public class CsvProcessRowInParallelTest extends CsvInputUnitTestBase {
  ...
  /**
   * Here files content is 16 bytes summary,
   * where 8 of this bytes is the first line,
   * 5 is the second one,
   * 3 is the last.
   *
   * As we are running this with 2 threads, we expect:
   * 1st thread to read 1st line
   * 2nd thread to read 2nd and 3d line.
   */
  @Test
  ...
  /**
   * So as not to heap up list of taken parameters, we are passing combi, but
   * we expect to see CsvInput class instances in it's content.
   */
