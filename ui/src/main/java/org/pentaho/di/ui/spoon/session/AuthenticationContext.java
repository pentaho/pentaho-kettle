...
public class AuthenticationContext {
  ...
  public boolean isSessionValid() {
    ...
    try ( Client client = createClient() ) {
      ...
      log.logError( "Session validation failed due to exception: " + e.getMessage() );
      return false;
    }
  }
  ...
  public boolean validateAndClearIfExpired() {
    ...
    if ( !isSessionValid() ) {
      ...
      // Remove the expired JSESSIONID entry from storage
      // This preserves the browser-auth marker for wasPreviouslyAuthenticated() check
      if ( strategy instanceof SessionBasedAuthStrategy ) {
        // For session-based strategy, explicitly store null to clear JSESSIONID while preserving auth marker
        storeCredentialValue( JSESSIONID_KEY, null );
      } else {
