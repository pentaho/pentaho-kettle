...
public class NamedConfigurationsController extends LazilyInitializedController implements IUISupportController {
  ...
  private void refreshNamedConfigurationList() {
    ...
    Runnable r = new Runnable() {
      public void run() {
        try {
          ObjectId[] ncIdList = repository.getNamedConfigurationIDs( false );
          for ( ObjectId ncId : ncIdList ) {
            NamedConfiguration namedConfiguration = repository.loadNamedConfiguration( ncId, null );
            try {
  ...
  public void createNamedConfiguration() {
    try {
      ...
      NamedConfiguration namedConfiguration = ConfigurationTemplateManager.getInstance().getConfigurationTemplates( 
          NamedConfigurationType.HADOOP_CLUSTER.getType() ).get( 0 );
      namedConfiguration.initializeVariablesFrom( null );
      ...
      if ( namedConfigurationName != null && !namedConfigurationName.equals( "" ) ) {
        ...
        ObjectId idnamedConfiguration = repository.getNamedConfigurationID( namedConfigurationName );
        if ( idnamedConfiguration == null ) {
          repository.insertLogEntry( BaseMessages.getString(
            PKG, "NamedConfigurationsController.Message.CreatingNamedConfiguration", getNamedConfigurationDialog()
              .getNamedConfiguration().getName() ) );
          repository.save( getNamedConfigurationDialog().getNamedConfiguration(), Const.VERSION_COMMENT_INITIAL_VERSION, null );
        } else {
  ...
  public void editNamedConfiguration() {
    try {
      ...
      if ( namedConfigurations != null && !namedConfigurations.isEmpty() ) {
        ...
        NamedConfiguration namedConfiguration = ( (UINamedConfiguration) namedConfigurations.toArray()[0] ).getNamedConfiguration();

        // Make sure this NamedConfiguration already exists and store its id for updating
        ObjectId idNamedConfiguration = repository.getNamedConfigurationID( namedConfiguration.getName() );
        if ( idNamedConfiguration == null ) {
          MessageBox mb = new MessageBox( shell, SWT.ICON_ERROR | SWT.OK );
          mb.setMessage( BaseMessages.getString(
          ...
          if ( namedConfigurationName != null && !namedConfigurationName.equals( "" ) ) {
            repository.insertLogEntry( BaseMessages.getString(
              PKG, "NamedConfigurationsController.Message.UpdatingNamedConfiguration", namedConfiguration.getName() ) );
            repository.save( namedConfiguration, Const.VERSION_COMMENT_EDIT_VERSION, null );
          }
        }
      } else {
        MessageBox mb = new MessageBox( shell, SWT.ICON_ERROR | SWT.OK );
        ...
        for ( Object obj : namedConfigurations ) {
          if ( obj != null && obj instanceof UINamedConfiguration ) {
            ...
            if ( idNamedConfiguration == null ) {
              ...
              repository.deleteNamedConfiguration( namedConfiguration.getName() );
            }
          }
        }
      } else {
