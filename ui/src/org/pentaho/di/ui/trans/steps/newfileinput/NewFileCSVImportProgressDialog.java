...
import org.pentaho.di.trans.steps.newfileinput.NewFileLine;
...
public class NewFileCSVImportProgressDialog {
   ...
   * database.
   */
  public NewFileCSVImportProgressDialog( Shell shell, NewFileInputMeta meta, TransMeta transMeta,
  ...
  private String doScan( IProgressMonitor monitor ) throws KettleException {
    ...
    for ( int i = 0; i < nrfields; i++ ) {
      NewFileInputField field = meta.inputFiles.inputFields[i];
    ...
    NewFileInputMeta strinfo = (NewFileInputMeta) meta.clone();
    for ( int i = 0; i < nrfields; i++ ) {
    ...
    line = NewFileInputUtils.getLine( log, reader, encodingType, fileFormatType, lineBuffer );
    fileLineNumber++;
    int skipped = 1;
    ...
    while ( !errorFound && line != null && ( linenr <= samples || samples == 0 ) && !monitor.isCanceled() ) {
      ...
      Object[] r =
          NewFileInputUtils.convertLineToRow( log, new NewFileLine( line, fileLineNumber, null ), strinfo, null, 0,
