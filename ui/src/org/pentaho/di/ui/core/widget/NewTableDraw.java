...
public class NewTableDraw extends Canvas {
  ...
  public NewTableDraw( Composite parent, PropsUI props, WizardPage wPage, Vector<NewFileInputFieldInterface> fields ) {
    ...
    addPaintListener( new PaintListener() {
      public void paintControl( PaintEvent e ) {
        NewTableDraw.this.paintControl( e );
      }
    } );
    ...
    addMouseMoveListener( new MouseMoveListener() {
      public void mouseMove( MouseEvent e ) {
        ...
        NewFileInputFieldInterface field = getFieldOnPosition( posx );
  ...
  private NewFileInputFieldInterface getFieldOnPosition( int x ) {
    for ( int i = 0; i < fields.size(); i++ ) {
      NewFileInputFieldInterface field = fields.get( i );
  ...
  private void setMarker( int x ) {
    ...
    if ( idx < 0 ) // Position is not yet in the list, the field is not deleted, but added
    {
      if ( highest_smaller >= 0 ) {
        ...
        NewFileInputFieldInterface prevfield = fields.get( highest_smaller );
        int newlength = prevfield.getLength() - ( x - prevfield.getPosition() );
        NewFileInputFieldInterface field = prevfield.createNewInstance( getNewFieldname(), x, newlength );
      ...
      if ( highest_smaller >= 0 ) {
        ...
        NewFileInputFieldInterface prevfield = fields.get( highest_smaller );
        NewFileInputFieldInterface field = fields.get( idx );
  ...
  public void clearFields() {
    fields = new Vector<NewFileInputFieldInterface>();
  }
