...
public class TableView extends Composite {
  private static Class<?> PKG = TableView.class; // for i18n purposes, needed by
  ...
  private Menu mRow;
  private SelectionAdapter lsRowInsBef, lsRowInsAft, lsClipAll, lsCol1, lsCol2, lsRowUp, lsRowDown, lsClear, lsCopyToAll, lsSelAll, lsUnselAll,
      lsPasteAll, lsCutAll, lsDelAll, lsKeep, lsFilter, lsEditUndo, lsEditRedo;
  ...
  public TableView(VariableSpace space, Composite parent, int style, ColumnInfo[] columnInfo, int nrRows, boolean readOnly, ModifyListener lsm,
      PropsUI pr) {
    ...
    lsKeyText = new KeyAdapter() {
      public void keyPressed(KeyEvent e) {
         ...
         * 
         * if (text!=null && !text.isDisposed()) right = e.keyCode ==
         * SWT.ARROW_RIGHT && last_carret_position==text.getText().length();
         */
     ...
     * There is support for keys like LEFT/RIGHT/UP/DOWN/HOME/END/etc It
     * presents us with a problem because we only get the position of the
     * row/column AFTER the other listener did it's job. Therefor we added
     * global variables prev_rownr and prev_colnr
     */
    ...
    KeyListener lsKeyTable = new KeyAdapter() {
      public void keyPressed(KeyEvent e) {
        ...
        // System.out.println("ctrl: "+ctrl+" shift: "
        // +shift+" keycode: "+e.keyCode+" character:"+e.character);
        ...
        if ((e.keyCode == SWT.ARROW_DOWN && !shift) || (e.keyCode == SWT.ARROW_UP && !shift) || (e.keyCode == SWT.HOME && !shift)
        ...
        if (activeTableColumn > 0) {
          boolean text_char = (e.character >= 'a' && e.character <= 'z') || (e.character >= 'A' && e.character <= 'Z')
              || (e.character >= '0' && e.character <= '9') || (e.character == ' ') || (e.character == '_') || (e.character == ',')
              || (e.character == '.') || (e.character == '+') || (e.character == '-') || (e.character == '*') || (e.character == '/')
              || (e.character == ';');
  ...
  public void sortTable(int sortField, boolean sortingDescending) {
    ...
    try {
      ...
      new ErrorDialog(this.getShell(), BaseMessages.getString(PKG, "TableView.ErrorDialog.title"), BaseMessages.getString(PKG,
          "TableView.ErrorDialog.description"), e);
    }
  }
  ...
  /*
   * Example: -----------------------------------------------------------------
   * Field in stream;Dimension field TIME;TIME DATA_TYPE;DATA_TYPE
   * MAP_TYPE;MAP_TYPE RESOLUTION;RESOLUTION START_TIME;START_TIME
   * -----------------------------------------------------------------
   * 
  ...
  private void keepSelected() {
    ...
    /*
     * try { table.getRow(); } catch(Exception e) // Index is too high: lower to
     * last available value { setPosition(table.getItemCount()-1, 1); }
     */
  ...
  private void edit(int rownr, int colnr, boolean select_text, char extra) {
    ...
    case ColumnInfo.COLUMN_TYPE_TEXT:
      editText(row, rownr, colnr, select_text, extra);
      break;
  ...
  private void editButton(TableItem row, int rownr, int colnr) {
    ...
    /*
     * button.addFocusListener(new FocusAdapter() { public void
     * focusLost(FocusEvent e) { button.dispose(); } } );
     */
  }
  ...
  /**
   * Count non-empty rows in the table... IMPORTANT: always call this method
   * before calling getNonEmpty(int selnr): for performance reasons we cache the
   * row indexes.
   * 
  ...
  /**
   * Return the row/table-item on the specified index. IMPORTANT: the indexes of
   * the non-empty rows are populated with a call to nrNonEmpty(). Make sure to
   * call that first.
   * 
   * @param index
