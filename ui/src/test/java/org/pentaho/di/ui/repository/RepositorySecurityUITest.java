...
public class RepositorySecurityUITest {
  ...
  @Test
  public void generalSecurityExceptionWithDisplayErrorTrueShowsErrorDialog() throws KettleException {
    KettleException securityException = new KettleException( "Not authorized" );
    doThrow( securityException )
      .when( mockSecurityProvider ).validateAction( any( RepositoryOperation[].class ) );

    try ( MockedConstruction<ErrorDialog> errorDialogMock = org.mockito.Mockito.mockConstruction( ErrorDialog.class ) ) {
      boolean result = RepositorySecurityUI.verifyOperations( mockShell, mockRepository, true,
        RepositoryOperation.MODIFY_TRANSFORMATION );

      assertTrue( result );
      assertEquals( 1, errorDialogMock.constructed().size() );
      verify( mockSpoon, never() ).handleRepositoryLost( any() );
    }
  }

  @Test
  public void buildOperationsDescriptionFormatsMultipleOperationsInErrorDialogMessage() throws KettleException {
    doThrow( new KettleException( "forbidden" ) )
      .when( mockSecurityProvider ).validateAction( any( RepositoryOperation[].class ) );

    java.util.List<String> capturedMessages = new java.util.ArrayList<>();
    try ( MockedConstruction<ErrorDialog> errorDialogMock = org.mockito.Mockito.mockConstruction( ErrorDialog.class,
        ( mock, context ) -> capturedMessages.add( (String) context.arguments().get( 2 ) ) ) ) {
      boolean result = RepositorySecurityUI.verifyOperations( mockShell, mockRepository, true,
        RepositoryOperation.MODIFY_TRANSFORMATION, RepositoryOperation.DELETE_JOB,
        RepositoryOperation.EXECUTE_TRANSFORMATION );

      assertTrue( result );
      assertEquals( 1, capturedMessages.size() );
      assertTrue( capturedMessages.get( 0 ).contains(
        "[Modify transformation, Delete job, Execute transformation]" ) );
    }
  }
}
