...
public class BrowserAuthenticationServiceTest {
  ...
  public void buildAuthenticationUrlIgnoresBlankAuthorizationUri() {
    ...
    assertTrue( url.contains( "/plugin/browser-auth/api/login?callback=" ) );
  }

  @Test
  public void getLocalCallbackHostReturnsFallbackWhenInetAddressThrows() {
    try ( MockedStatic<InetAddress> inetStatic = mockStatic( InetAddress.class ) ) {
      inetStatic.when( InetAddress::getLocalHost ).thenThrow( new UnknownHostException( "no dns" ) );
  ...
  @Test
  public void getLocalCallbackHostReturnsHostAddressWhenNotLoopback() {
    try ( MockedStatic<InetAddress> inetStatic = mockStatic( InetAddress.class ) ) {
      InetAddress mockAddress = mock( InetAddress.class );
      when( mockAddress.isLoopbackAddress() ).thenReturn( false );
      when( mockAddress.getHostAddress() ).thenReturn( "192.168.1.50" );
      inetStatic.when( InetAddress::getLocalHost ).thenReturn( mockAddress );
  ...
  @Test
  public void getLocalCallbackHostReturnsLocalhostWhenAddressIsLoopback() {
    try ( MockedStatic<InetAddress> inetStatic = mockStatic( InetAddress.class ) ) {
      InetAddress mockAddress = mock( InetAddress.class );
      when( mockAddress.isLoopbackAddress() ).thenReturn( true );
      inetStatic.when( InetAddress::getLocalHost ).thenReturn( mockAddress );
  ...
  public void openSystemBrowserFallsBackToOsWhenBrowseNotSupported() throws IOException {
    ...
    try ( MockedStatic<Desktop> deskStatic = mockStatic( Desktop.class );
          MockedStatic<Runtime> rtStatic = mockStatic( Runtime.class ) ) {
      Desktop mockDesktop = mock( Desktop.class );
      deskStatic.when( Desktop::isDesktopSupported ).thenReturn( true );
      deskStatic.when( Desktop::getDesktop ).thenReturn( mockDesktop );
      ...
      rtStatic.when( Runtime::getRuntime ).thenReturn( mockRuntime );

      service.openSystemBrowser( "http://localhost/test" );

      verify( mockRuntime ).exec( "rundll32 url.dll,FileProtocolHandler http://localhost/test" );
    } finally {
  ...
  public void openSystemBrowserExecutesCorrectCommandPerOs() throws IOException {
    Object[][] cases = {
      { "Windows 10", "rundll32 url.dll,FileProtocolHandler http://test/path" },
      { "Mac OS X",   "open http://test/path" },
      { "Linux",      "xdg-open http://test/path" }
    };

    String originalOs = System.getProperty( "os.name" );
    for ( Object[] tc : cases ) {
      String osName      = (String) tc[ 0 ];
      String expectedCmd = (String) tc[ 1 ];
    ...
    try ( MockedStatic<Desktop> deskStatic = mockStatic( Desktop.class );
      ...
      service.openSystemBrowser( "http://test/path" );
        ...
        verify( mockRuntime ).exec( expectedCmd );
    }
  }
      System.setProperty( "os.name", originalOs );
    }
  ...
  public void openSystemBrowserUsesXdgOpenForUnixOs() throws IOException {
    ...
    try ( MockedStatic<Desktop> deskStatic = mockStatic( Desktop.class );
      ...
      verify( mockRuntime ).exec( "xdg-open http://localhost/test" );
    } finally {
      System.setProperty( "os.name", originalOs );
    }
  }
