...
public class HadoopConfigurationUtil {
  ...
  private static final String CLASS_JOB_ENTRY_PLUGIN_TYPE = "org.pentaho.di.core.plugins.JobEntryPluginType";
  ...
  protected static ClassLoader findBigDataPluginClassLoader() throws Exception {
    ...
    try {
      Class<?> jobEntryPluginTypeClass = Class.forName(CLASS_JOB_ENTRY_PLUGIN_TYPE);
      Object hadoopCopyFilesPlugin = findPluginById.invoke(pluginRegistry, jobEntryPluginTypeClass,
          "HadoopCopyFilesPlugin");
      return (ClassLoader) getClassLoader.invoke(pluginRegistry, hadoopCopyFilesPlugin);
    } catch (Exception ex) {
      throw new Exception("Unable to locate Big Data Plugin's Hadoop Copy Files plugin", ex);
    }
  }

  protected static Object findHadoopConfigurationRegistry() throws Exception {
    try {
  ...
  protected synchronized static Object getRegistry() throws Exception {
